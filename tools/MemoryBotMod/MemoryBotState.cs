...
internal static class MemoryBotState
{
    ...
    private static string Game()
    {
        ...
        string regionRole = region != null ? region.Role().ToString() : "";
        string terrainId = region != null ? region.TerrainId : "";
        string templateId = region != null ? region.TemplateId : "";
        return "{\"scene\":" + Q(UnityEngine.SceneManagement.SceneManager.GetActiveScene().name)
            ...
            + ",\"region_role\":" + Q(regionRole)
            + ",\"terrain_id\":" + Q(terrainId)
            + ",\"template_id\":" + Q(templateId)
            + ",\"personal_region_id\":" + Q(GameManager.PersonalRegionId ?? "") + "}";
    }
    ...
    /// <summary>สูตรคราฟต์ที่ปลดล็อกแล้วฝั่งเกม — ใช้ยืนยันระบบปลดสูตรตามสกิล</summary>
    private static string Recipes(int limit)
    {
        ...
        if (system == null || system.RecipeContainer == null) return "{\"count\":0,\"total\":0,\"ids\":[]}";
        List<string> available = new List<string>();
        int total = 0;
        system.RecipeContainer.EnumerateRecipes(delegate(Crafting.Recipe recipe)
        {
            total++;
            if (recipe != null && recipe.Available) available.Add(recipe.Id);
        });
        StringBuilder sb = new StringBuilder("{\"count\":").Append(available.Count)
            .Append(",\"total\":").Append(total).Append(",\"ids\":[");
        for (int i = 0; i < available.Count && i < limit; i++)
        {
            if (i > 0) sb.Append(',');
            sb.Append(Q(available[i]));
        }
