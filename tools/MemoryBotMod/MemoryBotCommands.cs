...
internal static class MemoryBotCommands
{
    public static string Execute(MemoryBotRequest request)
    {
        ...
        if (request.Name == "ui.open")
        {
            ...
            if (uri == "Estate") { UIManager.Open<Durango.UI.EstateGroup>(); return Accepted(request.Name); }
            if (uri == "Character") { UIManager.Open<Durango.UI.CharacterInfoGroup>(); return Accepted(request.Name); }
            if (uri == "LearningGuide") { UIManager.Open<Durango.UI.LearningGuideGroup>(); return Accepted(request.Name); }
