# build-client.ps1 — build ตัวเกมจากซอร์สใน client\ แล้ววางลงในเกมให้เลย
#
...
#
# ทำไมถึงทำได้: client\ คือซอร์สที่ถอดจาก Assembly-CSharp.dll แล้ว **คอมไพล์ผ่าน**
# (client\Assembly-CSharp.csproj อ้าง DLL ของ Unity ในโฟลเดอร์ game\ โดยตรง) แก้ซอร์สตรง ๆ ได้เลย
#
...
$target  = Join-Path $root 'game\Durango_Data\Managed\Assembly-CSharp.dll'
$backups = Join-Path $root 'game-backup'
# Assembly-CSharp.dll อ้างอิง DurangoClientModSdk.dll (ProjectReference) — ต้องวางคู่กันใน Managed\
# เสมอ ไม่งั้น Mono resolve ไม่เจอตอนเกมรัน
$builtSdk  = Join-Path $client 'bin\Release\net35\DurangoClientModSdk.dll'
$targetSdk = Join-Path $root 'game\Durango_Data\Managed\DurangoClientModSdk.dll'
# ClientMethodOverrideManager ใช้ Harmony ตอนรัน — คอมไพเลอร์ก๊อป DLL ของแพ็กเกจมาไว้ข้าง
# Assembly-CSharp.dll ตอนติดตั้งจึงต้องก๊อปตามไปด้วย
$builtHarmony  = Join-Path $client 'bin\Release\net35\0Harmony.dll'
$targetHarmony = Join-Path $root 'game\Durango_Data\Managed\0Harmony.dll'

function Say($t, $c = 'Gray') { Write-Host $t -ForegroundColor $c }

function Stop-Game {
  $p = Get-Process Durango -ErrorAction SilentlyContinue
...
Say 'เปิดเกมได้เลย (เมนูข้อ 5) — ถ้าพังให้ย้อนกลับด้วย -Restore' 'DarkGray'
