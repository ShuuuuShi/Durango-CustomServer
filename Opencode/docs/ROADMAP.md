...
### ค้างจากงานกรง (ตรวจสอบเชิงโต้แย้งแล้วว่าเป็นปัญหาจริง) — ปิด 4 จาก 5

- ✅ `Cheats.MakeItem` ไม่ตั้ง `Item.Ext = Reins` ⇒ หน้าต่างเลือกสัตว์ว่างเปล่า — แก้แล้ว
- ✅ `PerformanceYaml.Rein` ขาด `vehicle_entity_type` / `size` — แก้แล้ว (ครบ 10 คีย์ 101 รายการ)
- ✅ `Player.Inventory.HandleUseItemMsg` ยังไม่รองรับการประทับบังเหียน — **แก้แล้ว**: เอาสัตว์เชื่อง
  ออกจากกรงได้เป็น "บังเหียนที่มีสัตว์อยู่ข้างใน" ตามของจริง กดใช้ (ผูกพัน/귀속) แล้วได้สัตว์เลี้ยง
  จริง (`TryImprintRein` ใน `Player.Domestication.cs` · เดิมยัดเข้า PetStore ทันทีตอนออกจากกรง)
- ✅ `PetTuning.LifeSpanDaysBase` หน่วยไม่ตรง — แก้แล้ว (`× SecondsPerDay` ก่อนใช้)
- ⬜ `PetFood` ขาดฟิลด์ฝั่งการเลี้ยง — เซิร์ฟใช้แค่ `vigor` แต่ข้อมูลจริงมี
  `decrease_domesticate_time(_ratio)` / `increase_domesticate_success_rate` / `decrease_grow_time(_ratio)`
  ซึ่งฝั่งเกมไม่อ่าน (เป็นของเซิร์ฟแท้) ⇒ ป้อนอาหารตอนฝึกยังไม่ลดเวลา/ไม่เพิ่มโอกาสสำเร็จ
  (ฝั่งเกมก็ไม่มีข้อความ "อาหารลดเวลา" ให้เห็น จึงไม่เร่ง)
