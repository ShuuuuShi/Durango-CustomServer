...
public class MapSystem : GameSystem<MapSystem>
{
	...
	private void OnReady()
	{
		...
		_poiUpdater.Init();
		for (int num = 0; num < TerrainMeta.Indicators.Count; num++)
		{
			Indicator indicator = TerrainMeta.Indicators[num];
	...
	private void OnPointUpdate(Points point)
	{
		...
		if (Points.HasHome() && HomeAssigned != null)
		{
			HomeAssigned();
		}
	}
	...
	private void DoWarp([NotNull] Func<ReplyMessageHandlerRegistrar> onFinished, string warpMsg)
	{
		...
		WarpTimer.Play(Connections.Frontend.Ping + 5f);
	...
	{
		if (TriedTravelToStableRegion != null)
		{
			TriedTravelToStableRegion(role);
