...
public class PersonalMaps : Singleton<PersonalMaps>
{
	...
	private IEnumerator CoCapture(Point2 minTile, Point2 maxTile, Action<float?> onProgress, Action<MemoryStream> onResult)
	{
		...
		using (BitmapCaptor bitmapCaptor = new BitmapCaptor(mainCamera))
		{
			...
			Vector3 worldPosStart = GetTotalCaptureSteps(humaneLeftBottom, humaneRightTop, worldIntervalX, worldIntervalY, out var xTotalStep, out var yTotalStep);
			...
			JpegCompressor jpegCompressor = JpegCompressor.Create(imageWidth, imageHeight, 90);
			for (int y = 0; y < yTotalStep; y++)
			{
				...
				while (i >= 0 && !IsCanceled && jpegCompressor != null)
				{
					if (!jpegCompressor.AddRow(bitmapBuffer, i * 3))
					{
						jpegCompressor.Release();
						jpegCompressor = null;
					}
					yield return null;
					i -= bitmapWidth;
			...
			else
			{
				if (jpegCompressor != null)
				{
					memoryStream = jpegCompressor.Finish();
				}
