...
public sealed class Artifact : ImmovableBase
{
	...
	static Artifact()
	{
		...
		{
			ArtifactStateChanged = null;
			ArtifactDisplayChanged = null;
		};
	}
	...
	public void SetArtifactState(ArtifactState state, double eventTime)
	{
		...
		if (ArtifactStateChanged != null)
		{
			ArtifactStateChanged(this);
		}
	...
	private void CheckDurabilityChanged(Gauge prevDurability, double eventTime)
	{
		...
		{
			if (DurabilityGaugeUpdated != null)
			{
				DurabilityGaugeUpdated(this);
			}
		}, delay);
	}
	...
	public void UpdateDisplay(ArtifactDisplay msg)
	{
		...
		RefreshArtifactLook();
		if (ArtifactDisplayChanged != null)
		{
			ArtifactDisplayChanged(this);
		}
	}
	...
	public void CheckPermissionForMe([NotNull] Action<bool> onCheck)
	{
		...
		if ((type == OwnerType.PersonalPlayer || type == OwnerType.Player) && GameSystem<SocialSystem>.Instance().IsFriend(estateInfo.License.OwnerId))
		{
			SocialSystem.GetMyFriendType(estateInfo.License.OwnerId, delegate(Messages.FriendType friendType2)
			{
				Shared.Player.FriendType friendType = friendType2._FriendType;
