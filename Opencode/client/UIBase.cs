...
public class UIBase : MonoBehaviour, IUriInvokable
{
	...
	public static UIBase CurrentUI
	{
		...
		private set
		{
			...
			if (flag2 != flag && FullscreenAnchorTypeChanged != null)
			{
				FullscreenAnchorTypeChanged(flag);
			}
	...
	{
		if (OnPreCloseUI != null)
		{
			bool res = false;
			OnPreCloseUI(ref res);
			if (res)
			{
				return true;
			}
	...
	public virtual bool Open()
	{
		...
		if (CurrentUI != null)
		{
			...
			if (uiType != UIType.Openable && uiType != UIType.Default)
			{
				return false;
			}
		}
		...
		if (_lockScreenOrientation)
		{
			...
			if (isLockScreenOrientation != IsLockScreenOrientation && OnScreenOrientationLock != null)
			{
				OnScreenOrientationLock();
			}
		}
		return true;
	}
	...
	protected void SetCurrencyList(IEnumerable<CurrencyData> currencies)
	{
		...
		{
			List<CurrencyData> list = new List<CurrencyData>(_currencyList);
			_currencyList.Clear();
			_currencyList.AddRange(currencies);
			CurrencyChanged(list, _currencyList);
		}
		else
		{
			_currencyList.Clear();
