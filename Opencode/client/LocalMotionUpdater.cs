...
public class LocalMotionUpdater
{
	...
	public void Motion(string motion, float time = 0f, float playbackRate = 1f, bool forceTransition = false, bool overrideIdleMotion = false, string equip = null, ItemColor color = default(ItemColor))
	{
		if (!string.IsNullOrEmpty(motion))
		{
			...
			ReservedMotion t = new ReservedMotion
			{
				Motion = motion,
	...
	{
		return new PlayerAnimationConditionArguments
		{
			Framework = (int)Player.CurrentWeaponFramework,
			StandState = (int)StandState,
			MoveSpeed = (int)Singleton<PlayerController>.Instance().GetCurrentMoveSpeed(),
			IsInWater = IsInWater,
			IsMoving = _movedByManual,
			IsSwimming = IsSwimming,
			IsWaterCarried = IsWaterCarried,
			IsBushWhack = Player.IsBushWhacking,
			IsTired = Player.IsTired,
			IsRoadRunning = Player.IsRoadRunning,
			IsSleep = IsSleep,
			IsNovice = IsNovice
		};
	}
	...
	{
		if (Player.IsAlive && Player.IsRiding && _rideState != RideMotionState.None)
		{
