...
public class RecipeSystem : GameSystem<RecipeSystem>
{
	...
	private static readonly BipartiteMatching BipartiteMatching = new BipartiteMatching();
	...
	private void OnRecipeListMsg(Recipes m, PacketHeader header)
	{
		...
		if (RecipeItemsUpdated != null)
		{
			RecipeItemsUpdated(RecipeType.Crafting);
		}
	}
	...
	private void OnBlueprintListMsg(ArtifactBlueprints m, PacketHeader header)
	{
		...
		if (RecipeItemsUpdated != null)
		{
			RecipeItemsUpdated(RecipeType.Building);
		}
	}
	...
	public bool CanCraftNow(CategoryItem categoryItem)
	{
		...
		if (categoryItem is Crafting.Recipe)
		{
			...
			return (!recipe.HasRequiredWorkbench || FindNearestAvailableWorkbench(recipe) != null) && HasMaterials(recipe);
		}
		if (categoryItem is Building.Blueprint)
		{
			Building.Blueprint blueprint = categoryItem as Building.Blueprint;
			return HasMaterials(blueprint);
		}
		return false;
	}
}
