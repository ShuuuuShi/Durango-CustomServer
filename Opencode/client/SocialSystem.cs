...
public class SocialSystem : GameSystem<SocialSystem>
{
	...
	private void Awake()
	{
		...
		{
			if (!GameManager.IsPrologueMode && GameManager.Emigrated != GameManager.EmigratedType.None)
			{
	...
	private void OnSubscriptionCount(SubscriptionCount msg, PacketHeader header)
	{
		...
		{
			SubscriptionCountChanged(msg.ChannelType);
		}
	}
	...
	private void OnJoinRecipients(JoinRecipients msg, PacketHeader header)
	{
		...
		if (conversation != null)
		{
			...
			if (RecipientsJoined != null)
			{
				RecipientsJoined(conversation.Id, msg.EntityIds);
			}
		}
	}
	...
	private void OnExitRecipient(ExitRecipient msg, PacketHeader header)
	{
		...
		if (conversation != null)
		{
			...
			OnConversationUpdate();
			if (RecipientExited != null)
			{
				RecipientExited(conversation.Id, msg.EntityId);
			}
		}
	}
	...
	{
		if (FollowerStatusChanged != null)
		{
			FollowerStatusChanged(msg.EntityId, msg.Followed);
		}
	}

	private void OnFollowingStatus(FollowingStatus msg, PacketHeader header)
	{
		if (FollowingStatusChanged != null)
		{
			FollowingStatusChanged(msg.EntityId, msg.Online);
		}
	}
	...
	private void OnFriendRequestAccepted(FriendRequestAccepted msg, PacketHeader header)
	{
		...
		{
			FriendRequestAccepted(msg.EntityId);
		}
	}
	...
	public void Say(string message, bool isDictation = false)
	{
		...
		SayInExclusiveChannel msg = new SayInExclusiveChannel
		{
		...
		if (!string.IsNullOrEmpty(message))
		{
			...
			Connection connection = ((GameManager.ClusterMode != Mode.Online) ? Connections.Frontend : Connections.Radiotower);
			connection.Send(msg);
			AddContinuousChatTime(channelType);
		}
	}
	...
	public void Say(string conversationId, string message, bool isDictation = false)
	{
		...
		SayInConversation msg = new SayInConversation
		{
			Message = new Message_
			{
				EntityId = GameManager.PlayerId,
	...
	public void SystemSay(object body, ChannelType? channelType = null, string conversationId = null)
	{
		...
		if (IsAllowedChannel(channelType.Value))
		{
			...
			if (channelType == ChannelType.Conversation)
			{
				...
				{
					Message = message,
					ConversationId = ((!string.IsNullOrEmpty(conversationId)) ? conversationId : _currentChannel.Id)
				};
			...
			{
				SayInExclusiveChannel msg2 = new SayInExclusiveChannel
				{
					Message = message,
	...
	public void PlayEmoticon(Emoticon emoticon)
	{
		...
		{
			PlayEmoticon msg = new PlayEmoticon
			{
				EntityId = GameManager.PlayerId,
				EmoticonId = emoticon.Key
			};
			Connections.Frontend.Send(msg);
			QuickSay($"[icon={emoticon.Icon}]");
			if (EmoticonPlayed != null)
			{
				EmoticonPlayed(emoticon.Key);
			}
		}
	}
