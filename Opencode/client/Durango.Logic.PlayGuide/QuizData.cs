...
public class QuizData
{
	...
	public string[] GetMessages(List<string> selected, out int index)
	{
		...
		{
			Event obj = _events[i];
			if (obj.Conditions.Length > selected.Count || KUtility.GetSize(obj.Messages) <= 0)
			{
				continue;
			}
			bool flag = true;
			int j = 0;
			for (int num = obj.Conditions.Length; j < num; j++)
			{
				if (obj.Conditions[j] != selected[j] && obj.Conditions[j] != "*")
				{
					flag = false;
					break;
				}
	...
	public static QuizData Parse(JObject obj)
	{
		...
		foreach (KeyValuePair<string, JToken> item in obj)
		{
			...
			obj2.Conditions = item.Key.SplitAndTrim('|');
			if (obj2.Conditions.Length == 0)
			{
				continue;
			}
			if (item.Value.Type == JTokenType.String)
			{
				obj2.Messages = new string[1] { item.Value.GetString() };
			}
			else
			{
				...
				obj2.Messages = item.Value.ToObject<string[]>();
			}
			obj2.Index = num;
			num++;
			quizData._events.Add(obj2);
		}
