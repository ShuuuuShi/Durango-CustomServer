...
public class PetResetRankPopup : TooltipBase
{
	...
	private void OnConfirm()
	{
		...
		if (!candidate.HasValue)
		{
			...
			messageBox.Show(T._("정말 진행하시겠습니까?"), T._("<alert>[icon=icon_make_alert] 재접속, 전투, 상점 등으로 화면이 전환되면 등급 초기화가 완료되지 않을 수 있습니다. 안전한 장소에서 진행하시는 것을 추천드립니다.</alert>"), delegate(bool ok)
			{
				...
				{
					PetManager.RevertPetRank(_pet.EntityId, delegate(RevertPetRankCandidate? candidate2)
					{
						_isWait = false;
						_candidate = candidate2;
						MarkAsChanged();
					});
				}
			});
	...
	private void ShowResultPage(bool instant)
	{
		...
		localPosition.x = 0f;
		Vector3 pos2;
		Vector3 pos = (pos2 = localPosition);
		...
		if (instant)
		{
			_rankWidgets[0].SetPosition(pos, useTween: false);
			_rankWidgets[1].SetPosition(pos2, useTween: false);
		...
		_rankWidgets[0].transform.localPosition = localPosition;
		_rankWidgets[0].SetPosition(pos);
		_rankWidgets[1].SetPosition(pos2, useTween: false);
