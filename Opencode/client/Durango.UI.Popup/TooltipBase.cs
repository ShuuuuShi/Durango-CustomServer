...
public abstract class TooltipBase : MonoBehaviour
{
	...
	protected virtual void Update()
	{
		...
		if (_markAsChanged != ChangedType.None)
		{
	...
	public void Show(UIWidget parent, Vector2 offset, float duration = 0f)
	{
		...
		Vector3 pos = UIUtility.ToRootPosition(pos: new Vector3(Mathf.Lerp(rect.xMin, rect.xMax, vector.x), Mathf.Lerp(rect.yMin, rect.yMax, vector.y)), parent: parent.gameObject);
		pos.x += offset.x;
		pos.y += offset.y;
		DoShow(pos, duration);
	}
	...
	{
		SelectableButton confirmButton = GetConfirmButton(out var showShortcut);
