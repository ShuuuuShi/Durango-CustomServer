...
public class PlayGuideSystem : GameSystem<PlayGuideSystem>
{
	...
	private void Awake()
	{
		...
		Connections.Frontend.On(delegate(ReturnerInfo msg, PacketHeader header)
		{
			...
			if (ReturnerUpdated != null)
			{
				ReturnerUpdated(msg.IsReturner);
			}
		});
	...
	private GuideStorageData LoadGuideStorageData(string key, [CanBeNull] Dictionary<string, byte[]> storage)
	{
		...
		if (UseLocalGuideProgress)
		{
			...
			guideStorageData = Json.Read<GuideStorageData>(json);
		}
	...
	private void SetCurrentEvent([NotNull] GuideEvent current)
	{
		...
		{
			EventChanged(currentEvent, _currentEvent);
		}
	...
	{
		if (guideEvent != null && (guideEvent.ToDoCollection == null || guideEvent.ToDoCollection.IsReady) && HelperTargetApplied != null)
		{
			HelperTargetApplied(guideEvent);
		}
	}
	...
	{
		if (HelperTargetRemoved != null)
		{
			HelperTargetRemoved(guideEvent);
		}
	}
	...
	{
		if (ExternalEventOccured != null)
		{
			ExternalEventOccured(type, param);
		}
	}
	...
	private void TerrainA6_OnLoadingChunksFinished()
	{
		...
		if (Begun != null)
		{
			Begun(_prevGuideRole, _currentGuideRole);
		}
