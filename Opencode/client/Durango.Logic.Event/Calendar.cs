...
public class Calendar
{
	...
	private void RefreshAppendicesStates()
	{
		...
		if (flag)
		{
			for (int num = 0; num < _appendices.Count; num++)
			{
				CalenderReward value = _appendices[num];
				value.State = RewardState.Completed;
				_appendices[num] = value;
			}
		}
		...
		for (int num2 = 0; num2 < _appendices.Count; num2++)
		{
			CalenderReward value2 = _appendices[num2];
			if (value2.State == RewardState.None)
			{
				value2.State = RewardState.Ready;
				_appendices[num2] = value2;
			}
		}
	}
