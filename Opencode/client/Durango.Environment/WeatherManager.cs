...
public class WeatherManager : Singleton<WeatherManager>
{
	...
	private void SetWeather(string weatherStr)
	{
		...
		{
			Debug.LogError("Invalid Weather string: " + weatherStr);
		}
		else if (weatherFromString != CurrentWeather)
		{
	...
	private IEnumerator SetCloudiness(float targetValue)
	{
		...
		while (true)
		{
			...
			float ratio = elapsed / _weatherChangeTransitionTime;
			colorCorrection.Cloudiness = Mathf.Lerp(startCloudiness, targetValue, ratio);
			if (ratio >= 1f)
			{
				break;
			}
