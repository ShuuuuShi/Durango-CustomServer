...
public class SoundBanksInfo
{
	...
	{
		foreach (SoundBanksInfoJson.Info.Bank.Event obj in includedEvents)
		{
			string bankPathFromObjectPath = GetBankPathFromObjectPath(obj.ObjectPath, bankPathDictionary);
			if (bankPathFromObjectPath == null)
			{
				throw new Exception($"이벤트 경로명으로부터 뱅크 경로명을 찾을 수 없습니다: {obj.ObjectPath}");
			}
			_eventNameToMediaIncludedBankPath[obj.Name] = bankPathFromObjectPath;
		}
	}
