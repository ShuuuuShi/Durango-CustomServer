...
public class BuildLocator : Singleton<BuildLocator>
{
	...
	public struct Arguments
	{
		...
		public static Arguments MakeFrom([NotNull] Blueprint blueprint)
		{
			return new Arguments
			{
				Blueprint = blueprint,
				Size = blueprint.Size,
				RotatableDirections = blueprint.RotatableDirections,
				BuildableBiomes = blueprint.BuildableBiomes,
				Exterior = blueprint.Exterior,
				Interior = blueprint.Interior,
				MinDepth = blueprint.MinBuildableDepth,
				MaxDepth = blueprint.MaxBuildableDepth,
				GetEffectTilesFunc = blueprint.GetEffectTiles
			};
		}
	}
	...
	private void Start()
	{
		...
		_previewModel.ModelLoaded += delegate(ModelComponent.IModel obj)
		{
			...
			{
				Renderer[] componentsInChildren = obj.GetObject().GetComponentsInChildren<Renderer>();
				Renderer[] array = componentsInChildren;
				foreach (Renderer renderer in array)
				{
					if ((renderer is MeshRenderer || renderer is SkinnedMeshRenderer) && renderer.sharedMaterials != null && !renderer.sharedMaterials.All((Material m) => m == null || m.renderQueue < 2000 || m.renderQueue > 2100))
					{
						renderer.sharedMaterials = new Material[1] { _previewMaterial };
					}
				}
			}
		};
	...
	private void LoadDefaultPreview()
	{
		...
		{
			ModularArtifact.FillModels(_previewModel, msg, _arguments.Size, _arguments.Stories ?? 1, _arguments.HasRoof, new Vector2(0.5f, 0.5f));
		}
		else
		{
	...
	public void RotatePreview()
	{
		if (_isPreviewVisible)
		{
			...
			if (!_isPresetPreview && _arguments.Blueprint.IsModular)
			{
				...
				ModularArtifact.FillModels(_previewModel, _arguments.Display.GetValueOrDefault(_arguments.Blueprint.GetDefaultDisplay()), size2, _arguments.Stories ?? 1, _arguments.HasRoof, new Vector2(0.5f, 0.5f));
			}
			UpdateTransform();
		}
	}
	...
	private BuildGridState GetTileBuildState(Point2 tile)
	{
		...
		if (artifact.BuildCompleted)
		{
			...
			if (artifact.IsOccupiablePos(pos))
			{
				...
				{
					if (artifact.GetInterior(pos, num2 + num3) != null)
					{
						flag3 = false;
						break;
					}
	...
	private void UpdateTransform()
	{
		...
		if (PreviewPositionUpdated != null)
		{
			PreviewPositionUpdated();
		}
	...
	public BuildSystem.GridResult GetResult()
	{
		...
		{
			Blueprint = _arguments.Blueprint,
			Tile = WorldTilePos,
			Size = _arguments.Size,
			Floor = _arguments.Floor,
			Stories = _arguments.Stories,
			Rotation = Rotation
		};
