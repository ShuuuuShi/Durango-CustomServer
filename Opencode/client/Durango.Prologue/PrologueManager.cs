...
public class PrologueManager : Durango.Utils.Singleton<PrologueManager>
{
	...
	private void UpdateState()
	{
		...
		case State.CreatePlayer:
		{
			...
			if (_curState > curState)
			{
				...
				{
					UIRect component = transform.GetChild(num).GetComponent<UIRect>();
		...
		for (int i = 0; i < num; i++)
		{
			...
			for (int j = 0; j < num2; j++)
			{
				...
				else if (material.HasProperty("_Wetness"))
				{
					...
					{
						float num3 = material.GetFloat("_MaxWetness");
						material.SetFloat("_Wetness", wetness * num3);
					}
					else
					{
	...
	private void RequestCreatePlayer()
	{
		...
		PlayerDisplay data = new PlayerDisplay
		{
			hair = display.Hair,
			body_color = display.BodyColor,
			head_color = display.HeadColor,
			skin_color = display.SkinColor,
			hair_color = display.HairColor,
			eye_color = display.EyeColor,
			lip_color = display.LipColor,
			portrait = display.Portrait,
			portrait_bg = display.PortraitBg,
			portrait_bg_color = display.PortraitBgColor,
			beard = display.Beard,
			voice_type = display.VoiceType,
			body_size = display.BodySize
		};
