...
public class CameraController : Singleton<CameraController>
{
	...
	protected override void OnAwake()
	{
		...
		_target = new Sequence<Target>(delegate(NgInterpolate.Function ease, Target begin, Target end, float time, float duration)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				value[i] = ease(vector[i], vector2[i] - vector[i], time, duration);
			}
			return new Target(value);
		}, (Target value) => new Target(value.Get()));
		_zoomRange = new Sequence<ZoomRange>((NgInterpolate.Function ease, ZoomRange begin, ZoomRange end, float time, float duration) => new ZoomRange
		{
			Min = ease(begin.Min, end.Min - begin.Min, time, duration),
			Max = ease(begin.Max, end.Max - begin.Max, time, duration)
		});
		...
		_zoomRatio.Value = 1f;
		_zoomRange.Value = new ZoomRange(0.42f, 2.2f);
