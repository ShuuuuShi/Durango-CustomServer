...
public class TextBuilder : IDisposable
{
	...
	private bool ProcessText(TextTokens tokens, TextTokens result, bool full, float scale, int maxLineCount, bool useEllipsis, bool wrapAlways)
	{
		...
		while (true)
		{
			...
			TextToken textToken = tokens[num9];
			TokenType type = textToken.Type;
			Vector2 vector = textToken.Size * num10;
			float num16 = textToken.Offset * num10;
			...
			if (textToken.IsSingle)
			{
				...
				if (num9 + 1 < tokens.Count && tokens[num9 + 1].Type != TokenType.Line)
				{
					flag2 = true;
					num4 = num9 + 1;
					flag3 = false;
			...
			if (textToken.IsLineSeperator())
			{
				num4 = ((textToken.Type != TokenType.Space) ? (num9 + 1) : num9);
				num5 = num9 + 1;
			}
		...
		if (!flag && useEllipsis && full && result.Count > 0 && tokens.Options.Count > 0)
		{
			...
			if (num17 < (float)Width)
			{
				...
				{
					TextToken textToken2 = result[num19];
					if (textToken2.Type == TokenType.Line)
					{
						break;
					}
					num17 -= (textToken2.Size * num10).x + num11;
					if (!(num17 > 0f))
					{
						num18 = num19;
						break;
				...
				if (num18 != -1)
				{
					...
					TextToken token3 = new TextToken
					{
						...
						Character = 46,
						PrevCharacter = 46,
						Size = new Vector2(glyph.advance, lastOption.FontSize)
					};
					for (int k = 0; k < 3; k++)
					{
						result.Add(token3);
	...
	public void Build(TextTokens tokens, Color color, float width, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols)
	{
		...
		for (int count = tokens.Count; i < count; i++)
		{
			...
			case TokenType.Character:
			{
				...
				Color color3 = color;
				if ((textOption.Effects & Effects.IgnoreColor) != Effects.None)
				{
					color3 = Color.white;
					color3.a = color.a;
				}
			...
			case TokenType.Link:
			{
				Color color2 = (((textOption.Effects & Effects.IgnoreColor) != Effects.None) ? textOption.Color : (textOption.Color * color));
