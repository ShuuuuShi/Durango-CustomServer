	...
	private void RefreshCurves()
	{
		if (_isDirtyCurves)
		{
			...
			for (int i = 0; i < points.Count; i++)
			{
				...
				vector3 /= magnitude;
				magnitude *= 0.5f;
				CurvePoint item = new CurvePoint
				{
					Pos = pos,
					Vector = vector3,
					Power = magnitude,
					Angle = Mathf.Repeat(Mathf.Atan2(pos.y, pos.x) * 57.29578f, 360f)
				};
				_curvePoints.Add(item);
			}
			for (int j = 0; j < _curvePoints.Count; j++)
			{
				CurvePoint curvePoint = _curvePoints[j];
				CurvePoint curvePoint2 = _curvePoints[(j + 1) % _curvePoints.Count];
				float num = Mathf.Min(curvePoint.Power, curvePoint2.Power);
				Maths.BezierCurve4 item2 = new Maths.BezierCurve4
				{
					P1 = curvePoint.Pos,
					P2 = curvePoint.Pos + curvePoint.Vector * num,
					P3 = curvePoint2.Pos - curvePoint2.Vector * num,
					P4 = curvePoint2.Pos
				};
				_curves.Add(item2);
			}
