...
public class MarketSystem : GameSystem<MarketSystem>
{
	...
	private void Awake()
	{
		Connections.Frontend.On(delegate(MarketCollectablePaymentExists msg, PacketHeader header)
		{
			...
			if (OnProductCollectiblePaymentExists != null)
			{
				OnProductCollectiblePaymentExists(HasCollectiblePayment);
			}
		});
		Connections.Frontend.On(delegate(ProductSold msg, PacketHeader header)
		{
			if (OnProductSold != null)
			{
				OnProductSold(msg);
			}
		});
		...
		{
			if (OnProductPaymentReceived != null)
			{
				OnProductPaymentReceived(msg);
			}
		});
		Connections.Frontend.On(delegate(ProductStateUpdated msg, PacketHeader header)
		{
			if (OnProductStateUpdated != null)
			{
				OnProductStateUpdated(msg);
			}
		});
	...
	{
		GetProducts(Send(new GetExpiredProducts
		{
			Skip = 0,
			Sort = null
		}), delegate
	...
	{
		Send(new RegisterMultipleProducts
		{
			ItemIds = Util.ItemsToIds(items),
			EachPrice = price,
			Duration = duration
		});
	}
	...
	public void BuyCommodity(string id, Action<bool> onResult)
	{
		...
		{
			if (Packet.IsSuccess(packet) && SuccessItemBuy != null)
			{
				SuccessItemBuy();
			}
			if (onResult != null)
	...
	{
		UnregisterProduct msg = new UnregisterProduct
		{
			ProductId = id
		};
	...
	{
		WithdrawProduct msg = new WithdrawProduct
		{
			ProductId = id
		};
		Send(msg).All(delegate(Packet packet)
	...
	public void ToggleFavorite(string commodityId, [NotNull] Action<bool, bool, string> favoriteAdded)
	{
		...
		ReplyMessageHandlerRegistrar searchProductsRegistrar = ((!isAddToFavorite) ? Send(new RemoveFromFavoriteProducts
		{
			ProductId = commodityId
		}) : Send(new AddToFavoriteProducts
		{
			ProductId = commodityId
		}));
