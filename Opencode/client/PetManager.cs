...
public class PetManager : Durango.Utils.Singleton<PetManager>
{
	...
	private void Start()
	{
		...
		{
			if (OnDomesticationResult != null)
			{
				OnDomesticationResult(msg);
			}
		});
		...
		Durango.Utils.Singleton<GameManager>.Instance().AddOnReady(delegate
		{
			...
			if (region.Role() == Role.Personal || GameManager.ClusterMode != Mode.Online)
			{
	...
	private void OnAppearPetMsg(AppearPet msg, PacketHeader header)
	{
		...
		bool flag = _pets.ContainsKey(msg.EntityId);
		ProcessPetMsg(msg.PetData.Value);
		if (!flag)
		{
			AddNewPet(msg.PetData.Value, msg.IsAlive);
			if (msg.PetData.Value.TamerEntityId == PlayerBehavior.LocalPlayer.EntityId)
			{
	...
	private void AddNewPet(Messages.Pet msg, bool isAlive)
	{
		...
		{
			if (_pets.TryGetValue(entityId, out var value))
			{
				bool flag = value.TamerEntityId == PlayerBehavior.LocalPlayer.EntityId;
				Vector3 spawnPosition = PlayerBehavior.LocalPlayer.transform.position + ((!flag || isAlive) ? new Vector3(20000f, 0f, 20000f) : Vector3.zero);
				AnimalBehavior animalBehavior = InstantiateAnimalObject(asset, spawnPosition, msg.EntityId, value.GetAnimalType(), isAlive);
				if (!(animalBehavior == null))
				{
					UpdatePet(value, animalBehavior);
					PetAI component = animalBehavior.GetComponent<PetAI>();
					if (!TrySetTamerPlayer(component, value))
					{
						_petsWaitingMaster.Add(value);
					}
					_petObjects[value.EntityId] = component;
					animalBehavior.ShowEyeTrail(value.HasEyeTrail());
					if (PetAppeared != null)
					{
						PetAppeared(animalBehavior);
					}
				}
			}
		});
	...
	private void ProcessPetMsg(Messages.Pet msg)
	{
		if (!string.IsNullOrEmpty(msg.EntityId))
		{
			...
			if (!string.IsNullOrEmpty(arg.EntityId) && PetChanged != null)
			{
				PetChanged(arg, msg);
			}
		}
	...
	private void PlayerManager_PlayerAppeared(PlayerBehavior player)
	{
		for (int num = _petsWaitingMaster.Count - 1; num >= 0; num--)
		{
			...
			if (!(msg.TamerEntityId != player.EntityId))
			{
	...
	private void UpdatePet(Messages.Pet msg, [NotNull] AnimalBehavior animal)
	{
		...
		PetAI component2 = animal.GetComponent<PetAI>();
		component2.SetHungryGauge(stat.Hungry);
	}

	[CanBeNull]
	public PetAI GetPetObject(string id)
	{
		return (!string.IsNullOrEmpty(id)) ? _petObjects.Get(id) : null;
	}
		...
		if (playerIncludeLocalPlayer != null)
		{
			...
			if (driver.Vehicle != null && driver.Vehicle.EntityId == msg.EntityId)
			{
				driver.ReturnVehicle(playReturnMotion: true, delegate
				{
					...
					if (PetDisappeared != null)
					{
						PetDisappeared(petObj.GetComponent<AnimalBehavior>());
					}
				});
				return;
			}
	...
	private void OnPetActiveSkillUsed(PetActiveSkillUsed msg, PacketHeader header)
	{
		...
		if (PetActiveSkillUsed != null)
		{
			PetActiveSkillUsed(msg);
		}
	}
	...
	private void OnPetActiveSkillCanceled(PetActiveSkillCanceled msg, PacketHeader header)
	{
		...
		if (PetActiveSkillCanceled != null)
		{
			PetActiveSkillCanceled(msg);
		}
	}
	...
	private void AddInterractionHandler()
	{
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.Mount, delegate(InteractionObject target)
		{
			...
			if (!(localVehicleBase == null))
			{
				AnimalBehavior component = localVehicleBase.GetComponent<AnimalBehavior>();
				if ((bool)component)
				{
					...
					{
						PetAI component2 = component.GetComponent<PetAI>();
						if (component2 != null && component2.Hungry == PetAI.HungryState.NoRide)
						{
							UIManager.SystemMsg(T._("동물이 배가 고파 탈 수 없습니다."));
							UIManager.Inventory.OpenPetFeedingPopup(component.EntityId);
						}
						else
						{
		...
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.ReturnPet, delegate(InteractionObject target)
		{
			...
			if (!(localVehicleBase == null))
			{
				ReturnMyPet(localVehicleBase.EntityId);
			}
		});
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.RenamePet, delegate(InteractionObject target)
		{
			...
			if (!(localVehicleBase == null))
			{
				AnimalBehavior animal = localVehicleBase.GetComponent<AnimalBehavior>();
				if ((bool)animal)
				{
		...
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.ResurrectPet, delegate(InteractionObject target)
		{
			...
			if (!(localVehicleBase == null))
			{
				...
				if ((bool)component)
				{
					ResurrectPet(component.EntityId);
				}
			}
		});
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.OpenPetMenu, delegate(InteractionObject target)
		{
			...
			if (!(localVehicleBase == null))
			{
				...
				if (!(component == null))
				{
					...
					petGroup.Open(component.EntityId);
				}
			}
		});
		...
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.AddProjectileToVehicle, delegate(InteractionObject target)
		{
			...
			if (!(artifact == null) && artifact.ArtifactState.Catapult.HasValue)
			{
				...
				{
					int i = 0;
					for (int size = KUtility.GetSize(catapult.RequiredProjectileTags); i < size; i++)
					{
						if (item.HasTag(catapult.RequiredProjectileTags[i]))
						{
							return true;
						}
					}
	...
	public static void SpawnMyPet(string id, Action onSuccess = null)
	{
		VehicleBase.RequestUnmountIfRiding(immediately: true, delegate
		{
			Driver component = PlayerBehavior.LocalPlayer.GetComponent<Driver>();
			if (!(component == null))
			{
	...
	{
		Connections.Frontend.Send(default(GetPetsInfo)).On(delegate(PetsInfo msg, PacketHeader header)
		{
			onResult(msg);
			if (!string.IsNullOrEmpty(GameManager.PersonalRegionId) && GameManager.Region.Id == GameManager.PersonalRegionId)
			{
				Durango.Utils.Singleton<PetManager>.Instance()._grazingPetManager?.Set(msg.GrazedPets.Data);
			}
		}).Rest(delegate
		{
			onResult(null);
	...
	private void AddPetContextAction(Messages.Pet? pet, List<InteractionMenuData> actions)
	{
		...
		for (int i = 0; i < availableActiveSkill.Length; i++)
		{
			...
			if (petActiveSkill2 != null && (petActiveSkill2.Context & currentSkillContext) != SkillContext.None)
			{
