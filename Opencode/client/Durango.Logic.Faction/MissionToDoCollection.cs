...
public class MissionToDoCollection : ToDoCollection
{
	...
	public void UpdateTodos(bool added)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			missionToDo2.ExpiresAt = ((!_mission.StartedAt.HasValue || !_mission.TimeLimit.HasValue) ? ((double?)null) : new double?(_mission.StartedAt.Value + (double)_mission.TimeLimit.Value));
