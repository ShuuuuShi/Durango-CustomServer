...
public class PlayerManager : Durango.Utils.Singleton<PlayerManager>
{
	...
	{
		AppearPlayer result = new AppearPlayer
		{
			EntityId = entityId,
			EntityType = (ushort)((!male) ? 1001 : 1000),
			IsAlive = true,
			Name = playerName,
			Title = 
			{
	...
	private IEnumerator Start()
	{
		...
		Connections.Frontend.On(delegate(AppearPlayer msg, PacketHeader header)
		{
			...
			if (flag)
			{
				...
				Vector3 pos = location.Position.ToClientPosition();
				pos.y = location.Height;
				Durango.Utils.Singleton<PlayerController>.Instance().Teleport(pos, TeleportType.Unknown, instance: true);
		...
		Connections.Frontend.On(delegate(Member msg, PacketHeader header)
		{
			...
			{
				SetClan(playerIncludeLocalPlayer, msg);
			}
		});
		Connections.Frontend.On(delegate(Messages.Title msg, PacketHeader header)
		{
			string entityId = msg.EntityId;
			PlayerBehavior playerIncludeLocalPlayer = GetPlayerIncludeLocalPlayer(entityId);
			if (playerIncludeLocalPlayer != null)
			{
				SetTitle(playerIncludeLocalPlayer, msg);
			}
		});
		Connections.Frontend.On(delegate(PlayerDisplay msg, PacketHeader header)
		{
			...
			if (playerIncludeLocalPlayer != null)
			{
				SetDisplay(playerIncludeLocalPlayer, msg, _hideOtherPlayer, fromAppear: false, handleBoarding: true);
				if (DisplayUpdated != null)
				{
					DisplayUpdated(playerIncludeLocalPlayer);
				}
			}
		});
		Connections.Frontend.On(delegate(VisualEffects msg, PacketHeader header)
		{
			...
			if (playerIncludeLocalPlayer != null)
			{
				playerIncludeLocalPlayer.SetParticleEffects(msg.Effects);
				playerIncludeLocalPlayer.SkinEffect = msg.SkinEffect;
			}
		});
		Connections.Frontend.On(delegate(PlayerDrawLine msg, PacketHeader header)
		{
			if (ShowDrawLine)
			{
				...
				if (!(player == null) && player.GetVisible() && !GameSystem<SocialSystem>.Instance().IsBlocked(player.EntityId) && !CombatSystem.IsHostilePlayer(player))
				{
					player.AddDrawLineBuffer(msg.DrawCommands);
				}
			}
		});
	...
	{
		player.Title = msg;
		if (PlayerTitleChanged != null)
		{
			PlayerTitleChanged(player);
		}
	}
	...
	private void SetClan(PlayerBehavior player, Member msg)
	{
		...
		{
			player.Clan = msg;
			if (PlayerClanChanged != null)
			{
				PlayerClanChanged(player);
			}
		}
	}
