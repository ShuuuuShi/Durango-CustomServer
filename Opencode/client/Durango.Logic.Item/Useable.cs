...
public static class Useable
{
	...
	private static void GetUsable(ItemData item, [NotNull] Inventory current, [CanBeNull] Inventory other, Inventory.InventoryMode mode, bool?[] result)
	{
		...
		if (other != null)
		{
			AddType(result, (current.Type != Inventory.InventoryType.Player) ? UseType.TakeOut : UseType.PutIn);
		}
		switch (mode)
		{
		...
		if (current.Type != Inventory.InventoryType.Player)
		{
			return;
		}
		...
		if (GameManager.ClusterMode != Mode.Online && PlayerBehavior.LocalPlayer != null)
		{
