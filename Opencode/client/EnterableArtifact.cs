...
public class EnterableArtifact : ArtifactComponent
{
	...
	public int Stories => base.Artifact.Stories.Value ?? 1;

	public bool HasRoof => base.Artifact.HasRoof.Value ?? true;
	...
	protected void AlphaTweenArtifactComponent(ModelComponent.IModel model, float alpha, float duration)
	{
		...
		AlphaTweenStruct alphaTweenStruct = new AlphaTweenStruct
		{
			Model = model,
			StartAlpha = model.GetColor().a,
			EndAlpha = alpha,
			StartAt = Time.time,
		...
		if (num == -1)
		{
			_alphaTweenList.Add(alphaTweenStruct);
		}
		else
		{
			_alphaTweenList[num] = alphaTweenStruct;
		}
	...
	private void CheckObstructions()
	{
		...
		for (int num = size.x + size.y + 1; i < num; i++)
		{
			...
			RaycastHit[] array = Collisions.RayCast(ray, direction.magnitude - 100f, mask, out var count);
