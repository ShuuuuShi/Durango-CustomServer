...
public class ObjectManager : Durango.Utils.Singleton<ObjectManager>
{
	...
	private void Start()
	{
		...
		{
			if (!_playerMgr.HandleMoveMsg(msg) && !_animalMgr.HandleMoveMsg(msg) && !_petMgr.HandleMoveMsg(msg))
			{
			}
		});
		Connections.Frontend.On(delegate(DisappearEntity msg, PacketHeader header)
		{
			if (!_playerMgr.HandleDisappearMsg(msg) && !_animalMgr.HandleDisappearMsg(msg) && !Durango.Utils.Singleton<ArtifactManager>.Instance().HandleDisappearMsg(msg))
			{
			}
		});
		...
		Connections.Frontend.On(delegate(Survival msg, PacketHeader header)
		{
			if (Application.isEditor && msg.EntityId == PlayerBehavior.LocalPlayer.EntityId)
			{
				...
				if (player != null)
				{
					player.SetSurvivalGauge(msg.Life, msg.Gauges);
				}
			}
			float timeDelay = 0f;
			...
			{
				StartCoroutine(ApplySurvivalGauge(characterBehavior, msg.Life, msg.Gauges, timeDelay));
			}
		});
		...
		Connections.Frontend.On(delegate(CombatInteraction msg, PacketHeader header)
		{
			if (msg.Details.ContainsKey("look_at"))
			{
				...
				if ((bool)gameObject)
				{
					BoneLookAtTarget component = gameObject.GetComponent<BoneLookAtTarget>();
					if (component != null)
					{
						...
						component.SetLookTarget(target, findHead: true);
					}
				}
			}
	...
	public GameObject FindObject(string id)
	{
		...
		return (!(artifact != null)) ? null : artifact.gameObject;
	}
	...
	public VehicleBase FindVehicle(string id)
	{
		...
		return (!(petObject != null)) ? null : petObject.GetComponent<VehicleBase>();
	}
	...
	public CharacterBehavior FindCharacter(string id)
	{
		...
		PetAI petObject = _petMgr.GetPetObject(id);
		return (!(petObject != null)) ? null : petObject.TargetAnimal;
	}
	...
	{
		ParticleEffect msg = new ParticleEffect
		{
			EntityId = entityId,
			Path = effect,
			Bone = bone,
