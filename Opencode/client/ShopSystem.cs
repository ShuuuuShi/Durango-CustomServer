...
public class ShopSystem : GameSystem<ShopSystem>
{
	...
	{
		string json = Preferences.GetString("read_commodities", string.Empty, Preferences.Level.User);
		string[] array = Json.Read<string[]>(json);
		...
		if (ReadCommoditiesChanged != null)
		{
			ReadCommoditiesChanged();
		}
	}
	...
	private void GetUserFirstPurchaseHistory()
	{
		Connections.Frontend.Send(default(GetUserFirstPurchaseHistory)).On(delegate(UserFirstPurchaseHistory msg, PacketHeader header)
		{
			...
			if (UserFirstPurchaseHistoryUpdated != null)
			{
				UserFirstPurchaseHistoryUpdated();
			}
		});
	}
	...
	private void OnAcceptableSubPurchases(AcceptableSubPurchases msg, PacketHeader header)
	{
		...
		if (AcceptableSubPurchasesUpdated != null)
		{
			AcceptableSubPurchasesUpdated();
		}
		if (AcceptableSubPurchaseItem != null)
		{
			for (int k = 0; k < list.Count; k++)
			{
				NewAcceptablePurchase newAcceptablePurchase = list[k];
				AcceptableSubPurchaseItem(newAcceptablePurchase.PurchaseId, newAcceptablePurchase.CommodityId, newAcceptablePurchase.SubId);
			}
		}
	}
	...
	{
		SetSpecialDeals(msg);
		if (NewSpecialDealCommoditiesReleased != null)
		{
			NewSpecialDealCommoditiesReleased();
		}
	}
	...
	{
		TransferDurangoCoin msg = new TransferDurangoCoin
		{
			Amount = amount,
			RecipientEntityId = targetPlayerId
		};
