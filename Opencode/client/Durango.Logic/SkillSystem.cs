...
public class SkillSystem : GameSystem<SkillSystem>
{
	...
	private void OnChangePlayerLevel(int prev, int level)
	{
		...
		for (int count = SkillCategories.Count; i < count; i++)
		{
			...
			if (category.Level >= prev && category.IsReadyToResearch() && CategoryResearchReady != null)
			{
				CategoryResearchReady(category);
			}
		}
	}
	...
	private void RaiseSkillEvent()
	{
		...
		for (int i = 0; i < _eventedSkills.Count; i++)
		{
			...
			if (key == SkillEventType.LevelChanged && SkillLevelChanged != null)
			{
				SkillLevelChanged(value);
			}
		}
		for (int j = 0; j < _eventedCategories.Count; j++)
		{
			...
			case CategoryEventType.LevelChanged:
				if (CategoryLevelChanged != null)
				{
					CategoryLevelChanged(value2);
				}
				break;
			case CategoryEventType.ResearchStarted:
				if (CategoryResearchStarted != null)
				{
					CategoryResearchStarted(value2);
				}
				break;
			case CategoryEventType.ResearchFinished:
				if (CategoryResearchFinished != null)
				{
					CategoryResearchFinished(value2);
				}
				break;
			case CategoryEventType.ResearchReady:
				if (CategoryResearchReady != null)
				{
					CategoryResearchReady(value2);
				}
				break;
			case CategoryEventType.ResearchUpdated:
				if (CategoryResearchUpdated != null)
				{
					CategoryResearchUpdated(value2);
				}
				break;
			}
		}
	...
	{
		if (SkillCategoryExperienced != null)
		{
			SkillCategoryExperienced(msg);
		}
	}
	...
	public void UntrainSkill([NotNull] Durango.Logic.Skill.Skill skill, string voucherId, Action<bool> onResult)
	{
		...
		UntrainSkill msg = new UntrainSkill
		{
			SkillId = skill.Id,
			SubId = skill.SubId,
			Level = skill.Level,
