...
public class PartySystem : GameSystem<PartySystem>
{
	...
	private void OnParty(Messages.Party msg, PacketHeader header)
	{
		...
		if (!info.HasValue || msg.Info.Value.MemberStatus == null)
		{
			...
			if (LeaderChanged != null && leaderInfo != null)
			{
				LeaderChanged();
			}
			return;
		...
		if (LeaderChanged != null && (leaderInfo == null || member.EntityId != leaderInfo.EntityId))
		{
			LeaderChanged();
		}
		if (isInvited != flag && flag && Invited != null)
		{
			Invited();
		}
	}
	...
	private void OnPartierStatus(PartierStatus msg, PacketHeader header)
	{
		...
		if (PartierStatusUpdated != null)
		{
			PartierStatusUpdated();
		}
	}
	...
	private void TestParty(bool invite, bool accept)
	{
		...
		if (invite)
		{
			...
			PartierStatus item = new PartierStatus
			{
