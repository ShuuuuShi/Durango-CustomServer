...
public class ExploreSystem : GameSystem<ExploreSystem>
{
	...
	private void AddToRecentlyVisits([NotNull] RegionTemplate template)
	{
		...
		string value = RecentlyVisits.FirstOrDefault(delegate(string key)
		{
			RegionTemplate regionTemplate = SingletonDict<string, RegionTemplate>.Get(key);
	...
	{
		string text = Preferences.GetString("RecentlyVisitTemplateIds", string.Empty, Preferences.Level.Player);
		RecentlyVisits = ((!string.IsNullOrEmpty(text)) ? Json.Read<LinkedList<string>>(text) : new LinkedList<string>());
	}
	...
	private void OnRoutesOfArchipelago(RoutesOfArchipelago archipelagoRoutes, PacketHeader header)
	{
		...
		{
			_Routes = null,
			ArchipelagoRoutes = archipelagoRoutes.ArchipelagoRoutes
		};
		OnRoutes(routes, header);
	}
	...
	private void OnRoutes(Routes routes, PacketHeader header)
	{
		...
		if (_routes != null)
		{
			foreach (KeyValuePair<Role, Dictionary<string, Route[]>> route2 in _routes)
			{
				Role key = route2.Key;
				foreach (KeyValuePair<string, Route[]> item in route2.Value)
				{
					...
					if (regionTemplate != null)
					{
						...
						{
							Route route = array[i];
							discoveredRegionIds.Add(route.RegionId);
						}
					}
				}
			}
		...
		for (int j = 0; j < archipelagoRoutes.Length; j++)
		{
			...
			if (archipelagoRoute.IncludedRoutes != null)
			{
				...
				discoveredRegionIds.AddRange(archipelagoRoute.IncludedRoutes.Select((Route route2) => route2.RegionId));
			}
		}
		_regions.Clear();
		...
		RequestArchipelagos(list, delegate(Archipelago[] archipelagoes)
		{
			...
			RequestRegionsForExploreSystem(discoveredRegionIds, RoutesUpdated);
		});
	}
