...
public class GrowCageGroup : UIBase
{
	...
	private void SelectPet(Messages.Pet? pet)
	{
		if (pet.HasValue)
		{
			...
			TaskStatus? task = (growCage.HasValue ? growCage.Value.GetTaskStatus(id) : ((TaskStatus?)null));
	...
	private void OnStartTask(Messages.Pet target, PetTaskType taskType)
	{
		...
		if (taskType != PetTaskType.Production)
		{
	...
	private void OnSkipTaskCheat(Messages.Pet pet)
	{
		...
		GrowCage? growCage = PetUtil.GetGrowCage(_cage);
		TaskStatus? taskStatus = (growCage.HasValue ? growCage.Value.GetTaskStatus(pet.EntityId) : ((TaskStatus?)null));
		if (taskStatus.HasValue)
		{
