...
public class MusicList : MonoBehaviour
{
	...
	private void ImportFromFile()
	{
		...
		SearchOption option = ((GameManager.ClusterMode != Mode.Online) ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly);
	...
	private void ImportMidi(Stream stream)
	{
		...
		if (timbre.Count > 1)
		{
			...
			{
				KeyValuePair<int, int?> keyValuePair = array2[num];
			...
			genericSelector.SetSelected(delegate(int[] selected)
			{
				if (KUtility.GetSize(selected) != 0)
				{
					...
					foreach (int num2 in selected)
					{
						hashSet.Add(array[num2].Key);
					}
					import(hashSet);
				}
			});
