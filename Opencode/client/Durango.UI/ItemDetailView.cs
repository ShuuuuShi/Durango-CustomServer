...
public class ItemDetailView : KWidgetScrollView
{
	...
	private static IEnumerable<TagData> EnumerateAdjustedItemTags(IEnumerable<TagData> itemTags, IEnumerable<ReformSlot> reformSlots)
	{
		...
		foreach (TagData tag2 in itemTags)
		{
			if (_reformSlotTagLevels.TryGetValue(tag2.Id, out var reformTagLevel))
			{
				if (tag2.Level - reformTagLevel > 0)
				{
					yield return TagData.Create(tag2.Id, tag2.Level - reformTagLevel);
				}
			}
			else
			{
