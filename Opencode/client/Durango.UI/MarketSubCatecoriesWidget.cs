...
public class MarketSubCatecoriesWidget : MonoBehaviour
{
	...
	private void SetCategory([CanBeNull] Category.Main main, bool isSelected, [CanBeNull] Category.Sub current)
	{
		...
		component.Clicked = delegate
		{
			...
			{
				nodes[i].GetComponent<SelectableWidget>().Selected = nodes[i] == firstNode;
			}
		...
		int num = 0;
		for (int size = KUtility.GetSize(category.Subs); num < size; num++)
		{
			Category.Sub targetData = category.Subs[num];
			...
			component3.Clicked = delegate
			{
				for (int i = 0; i < nodes.Count; i++)
				{
					nodes[i].GetComponent<SelectableWidget>().Selected = nodes[i] == targetNode;
				}
				if (SubCategorySelected != null)
				{
					SubCategorySelected(targetData);
				}
			};
		}
		nodes.EndLoad();
