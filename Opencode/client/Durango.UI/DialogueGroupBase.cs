...
public class DialogueGroupBase : UIBase
{
	...
	{
		bool pause = Current != null && Current.Type != Type.System && (Current.Portrait != null || !string.IsNullOrEmpty(Current.Image));
	...
	private void Refresh(bool resume = false)
	{
		...
		if (!resume || Current.Type != Type.System)
		{
	...
	private void SetQuiz([NotNull] Context ctx)
	{
		...
		for (int i = 0; i < choiceCount; i++)
		{
			...
			choiceButton.Clicked = delegate
			{
				...
				string[] messages = quiz.GetMessages(_quizSelected, out var index);
