...
public class CommodityListWidget : AnimationWidget, IUIInitializable, IScreenResizeReceiver
{
	...
	private void ShowPage(int index, bool instant)
	{
		...
		{
			int num = 0;
			num = ((!UIManager.IsPortraitWidget(base.gameObject) && _commodities.SearchOption != null && _commodities.SearchOption.MainCategory != null) ? (base.Widget.width - _subCategoriesWidget.Widget.width - _mainScrollView.Margin) : base.Widget.width);
	...
	private void OnBuyCommodity()
	{
		...
		if (item != null)
		{
			...
			{
				CommodityBought(ok: true);
				UIManager.SystemMsg(T._("<em>{0}</em>{0:-을} 받았습니다.", item.Name));
			}
		}
	}
