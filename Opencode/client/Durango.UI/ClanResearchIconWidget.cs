...
public class ClanResearchIconWidget : UIWidget
{
	...
	private void OnClick()
	{
		...
		State state = GetState();
		string text = $"<em>{_researchYaml.Name}</em>";
		...
		case State.Ready:
			...
			text2 = T._("고급 연구소에서 사용할 수 있습니다.");
			subtitle = string.Empty;
			break;
		case State.Active:
			widgetTooltipControl = UIManager.Popup.Tooltip<WidgetTooltipControl>();
			text2 = ClanResearchPopup.GetStatusEffectText(_researchYaml.Effect);
			subtitle = new SyncString(delegate(out string reference, out float period)
			{
				double num = _research.Until - Connections.Frontend.GetPredictedServerTime();
				reference = TimedeltaFormatter.Format(num);
		...
		case State.Cooltime:
			...
			subtitle = new SyncString(delegate(out string reference, out float period)
			{
				double num = _research.CooltimeUntil - Connections.Frontend.GetPredictedServerTime();
				reference = TimedeltaFormatter.Format(num);
				period = (float)(num % (double)TimedeltaFormatter.CurrentMinUnit());
			});
			break;
		...
		widgetTooltipControl.Set(text, subtitle, text2, 500);
