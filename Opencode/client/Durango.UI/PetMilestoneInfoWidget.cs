...
public class PetMilestoneInfoWidget : UIWidget, IUIInitializable
{
	...
	void IUIInitializable.Init()
	{
		...
		{
			if (MilestoneHelpClicked != null)
			{
				MilestoneHelpClicked(_pet);
			}
		});
	...
	private void OnClickGetMilestone(GameObject obj)
	{
		...
		if (focusedMilestone.HasValue && _pet.Statistics.Level >= _focusedMilestone.Value.Level && MilestonePicked != null)
		{
			MilestonePicked(_pet, _focusedMilestone.Value.MilestoneTableId);
		}
	}

	private void OnClickLearnActiveAction(GameObject obj)
	{
		if (ActiveSkillPicked != null)
		{
			ActiveSkillPicked(_pet);
		}
	}
