...
public class EstateInfoWidget : MonoBehaviour
{
	...
	private void UpdateBottomLabels(int size, int largestSize, EntityTile? warpholeTile)
	{
		...
		else
		{
			...
			component2.SetKey(T._("1일 유지비 <help>{0}</help>", (_license.Type != OwnerType.Player) ? T._("부족 영토의 면적에 비례하여 1일 유지비가 결정됩니다. 유지비는 지속적으로 예치금에서 차감됩니다.") : T._("사유지의 면적에 비례하여 1일 유지비가 결정됩니다. 유지비는 지속적으로 예치금에서 차감됩니다.")));
			...
			component3.SetKey(T._("예치금 <help>{0}</help>", (_license.Type != OwnerType.Player) ? T._("부족 영토의 면적과 예치금 액수에 따라 부족 영토 기간이 결정됩니다. 예치금을 늘리려면 우측의 부족 영토 기간 연장 버튼을 누르세요.") : T._("사유지의 면적과 예치금 액수에 따라 사유지 기간이 결정됩니다. 예치금을 늘리려면 우측의 사유지 기간 연장 버튼을 누르세요.")));
