...
public class MapContext : Singleton<MapContext>
{
	...
	private void OnRender_MapTexture(Material mat)
	{
		...
		{
			mat.SetVector("_RevealRange", new Vector4
			{
				x = (_revealCenter.x - 10f) / (float)MapSize,
				y = (_revealCenter.y - 10f) / (float)MapSize,
				z = (_revealCenter.x + 10f) / (float)MapSize,
				w = (_revealCenter.y + 10f) / (float)MapSize
			});
