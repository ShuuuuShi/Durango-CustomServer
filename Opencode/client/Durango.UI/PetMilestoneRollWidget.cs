...
public class PetMilestoneRollWidget : MonoBehaviour, IUIInitializable
{
	...
	public void Show(MilestoneCandidates candidates)
	{
		...
		for (int j = 0; j < array.Length; j++)
		{
			...
			{
				TagGrade tagGrade = (SingletonDict<string, Yaml.Tag>.TryGetValue(pair.Item1, out var value) ? value.Grade : TagGrade.Negative);
	...
	private IEnumerator RollAnimationCoroutine(float duration)
	{
		...
		{
			float now = Time.time;
			float ratio = Mathf.Clamp01((now - start) / (end - start));
			ratio = 1f - (1f - ratio) * (1f - ratio);
	...
	private IEnumerator RollCoroutine([NotNull] Action<Action<object>> requestResult)
	{
		...
		while (true)
		{
			...
			for (int num3 = 0; num3 < _rollItems.Count; num3++)
			{
				RollItem value = _rollItems[num3];
			...
			_stopRollFlag = null;
			for (int num4 = 0; num4 < _rollItems.Count; num4++)
			{
				RollItem item = _rollItems[num4];
		...
		{
			if (MilestoneRollFinished != null)
			{
				MilestoneRollFinished(result.Value.Milestone.Value);
			}
		}
		else if (result.Value.Skill.HasValue && DrawSkillRollFinished != null)
		{
			DrawSkillRollFinished(result.Value.Skill.Value);
		}
	}
	...
	private void OnItemFocused(RollItem? item)
	{
		if (item.HasValue)
		{
			...
			if (!string.IsNullOrEmpty(item.Value.Skill.SkillId))
			{
				...
				{
					SkillFocused(item.Value.Skill);
				}
				return;
			}
		}
		if (EmptyFocused != null)
		{
			EmptyFocused();
		}
	}
