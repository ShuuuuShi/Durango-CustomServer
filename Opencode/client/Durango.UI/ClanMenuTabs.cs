...
public class ClanMenuTabs : UIWidget
{
	...
	{
		if (index >= 0 && index < KUtility.GetSize(_menus) && Selected != null)
		{
			Selected(_menus[index]);
		}
	}
	...
	private void UpdateNotification()
	{
		...
		for (int size = KUtility.GetSize(_menus); i < size; i++)
		{
			if (_menus[i] == ClanGroup.ClanMenus.Members)
			{
				...
				bool flag = false;
				if (playerClan != null && !string.IsNullOrEmpty(clan.ClanId) && playerClan.TryGetRole(clan.RoleId, out var role) && (role.GetPermissions() & Permissions.ApproveMember) != Permissions.None)
				{
					flag = KUtility.GetSize(playerClan.Appliers) > 0;
				}
				_tabList.SetNotification(i, flag, Durango.Logic.Notification.Type.Normal);
			}
		}
	}
}
