...
public class BattleActionButtonContainer : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		{
			if (PlayerActionPressed != null)
			{
				PlayerActionPressed(btn, press);
			}
		};
		...
		{
			if (PetActionPressed != null)
			{
				PetActionPressed(btn, press);
			}
		});
		InitializeButton(_tameActinoSlot, delegate(BattleActionButton btn, bool press)
		{
			if (TameActionPressed != null)
			{
				TameActionPressed(btn, press);
			}
		});
	}
	...
	{
		GameObject gameObject = base.gameObject.AddChild(_actionButtonBase.gameObject);
		Transform transform = gameObject.transform;
		transform.localPosition = args.Position;
		transform.localScale = Vector3.one * args.Scale;
		BattleActionButton component = gameObject.GetComponent<BattleActionButton>();
	...
	public void SetPetAction(Messages.PetActiveSkill? action)
	{
		...
		PetSkillStates playerPetSkillStates = Durango.Utils.Singleton<PetManager>.Instance().PlayerPetSkillStates;
		PetSkillStates.State state = playerPetSkillStates.GetState(action.Value.SkillId);
		if (state == null)
		{
			_petActionSlot.Button.SetEmpty();
			return;
