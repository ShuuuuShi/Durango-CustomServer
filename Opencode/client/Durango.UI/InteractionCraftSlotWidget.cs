...
public class InteractionCraftSlotWidget : UIWidget
{
	...
	private void OnLongPress()
	{
		...
		{
			Messages.Crafting? crafting = _crafting;
			if (crafting.HasValue && CancelModeChanged != null)
			{
				CancelModeChanged(!_isCancelMode);
			}
		}
	}
	...
	private void TakeCrafted()
	{
		...
		CraftedResult crafted2 = _crafted.Value;
		string[] items = crafted2.Items.Select((Item item) => item.Id).ToArray();
	...
	{
		Messages.Crafting? crafting = _crafting;
		if (!crafting.HasValue || _crafting.Value.SkipCost == null)
		{
			return;
		}
		InteractionObject lastInteractionTarget = GameSystem<InteractionSystem>.Instance().LastInteractionTarget;
		...
		Messages.Crafting crafting2 = _crafting.Value;
		MessageBox messageBox = UIManager.MessageBox;
		int num = Mathf.FloorToInt(crafting2.SkipCost.Get());
		...
		messageBox.ShowPayConfirm(num, Currency.Gem, T._("즉시 완료하시겠습니까?"), delegate(bool ok)
		{
			if (ok)
			{
				CraftSystem.SkipEntrustedCraft(prop, crafting2.Id, delegate(bool success)
				{
					if (success)
					{
	...
	private void CancelCrafting()
	{
		...
		UIManager.MessageBox.Show(T._("진행 중인 제작을 취소하시겠습니까? 취소하면 제작 중인 물건은 사라집니다"), delegate(bool ok)
		{
			if (ok)
			{
				CraftSystem.CancelCrafting(prop, crafting2.Id, delegate(bool success)
				{
					if (success)
					{
						GameSystem<InteractionSystem>.Instance().SendTouchMsg();
