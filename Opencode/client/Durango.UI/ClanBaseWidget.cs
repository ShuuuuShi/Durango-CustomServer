...
public class ClanBaseWidget : UIWidget
{
	...
	private void RefreshBattleInfo(ClanCargoWarphole cargo)
	{
		...
		ClanBattle battleInfo2 = cargo.BattleInfo.Value;
		_cycleCountLabel.text = T._("{0} 회차", $"[b][i]{battleInfo2.Cycle}[/i][/b][size=20]");
		if (battleInfo2.CycleProtectionUntil > predictedServerTime)
		{
			...
			{
				double num = battleInfo2.CycleProtectionUntil - Connections.Frontend.GetPredictedServerTime();
				if (num > 0.0)
				{
		...
		_stateChangeAtTimeLabel.SetText(new SyncString(delegate(out string text, out float period)
		{
			double num = battleInfo2.CycleUntil - Connections.Frontend.GetPredictedServerTime();
			if (num > 0.0)
			{
				text = TimedeltaFormatter.Format(num);
				period = (float)(num % (double)TimedeltaFormatter.CurrentMinUnit());
