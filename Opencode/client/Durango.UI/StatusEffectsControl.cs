...
public class StatusEffectsControl : MonoBehaviour
{
	...
	protected virtual void RefreshStatusEffect(bool instant)
	{
		...
		case StatusType.Player:
		{
			StatusEffects statusEffects2 = GameSystem<StatusEffectSystem>.Instance().GetStatusEffects();
			for (int num2 = statusEffects2.List.Count - 1; num2 >= 0; num2--)
			{
				StatusEffect statusEffect2 = statusEffects2.List[num2];
				if (!statusEffect2.Template.Service)
				{
					SetStatusEffect(statusEffect2, ref index, tween);
				}
			}
			StatusEffects statusEffects3 = GameSystem<StatusEffectSystem>.Instance().GetStatusEffects(Singleton<PetManager>.Instance().GetPlayerPetId());
			if (statusEffects3 != null)
			{
				for (int num3 = statusEffects3.List.Count - 1; num3 >= 0; num3--)
				{
					StatusEffect status = statusEffects3.List[num3];
		...
		case StatusType.Target:
		{
			StatusEffects statusEffects4 = GameSystem<StatusEffectSystem>.Instance().GetStatusEffects(_target);
			if (statusEffects4 != null)
			{
				for (int num4 = statusEffects4.List.Count - 1; num4 >= 0; num4--)
				{
					StatusEffect status2 = statusEffects4.List[num4];
		...
		if (_scrollView != null)
		{
			...
			UIWidget uIWidget = scrollView.Widgets.First();
			uIWidget.width = _statusEffectBase.width * GetIconColumnCount();
	...
	private StatusEffectIcon StatusEffectIconPop()
	{
		...
		{
			GameObject gameObject = _statusEffectBase.transform.parent.gameObject.AddChild(_statusEffectBase.gameObject);
			statusEffectIcon = gameObject.GetComponent<StatusEffectIcon>();
	...
	private void SetStatusEffect(StatusEffect status, Color col, ref int index, bool tween)
	{
		...
		bool flag = !string.IsNullOrEmpty(status.Template.UIGroup);
		StatusEffectIcon statusEffectIcon2 = ((!flag) ? FindStatusEffectIcon(status.Id) : FindStatusEffectGroupIcon(status.Template.UIGroup));
	...
	private void ShowStatusEffectDescription(StatusEffectIcon status, float duration)
	{
		if (!(status == null) && status.Data != null && _parent.Visible && !(duration <= 0f))
		{
			...
			string text = ((GameSystem<StatusEffectSystem>.Instance().GetStatusEffects().GetStatusEffect(se.Id) == null) ? string.Format("[{1}]{0}[-]", se.Name, NGUIText.EncodeColor24(PresetColor.UIPet)) : $"<em>{se.Name}</em>");
			...
			SyncString subtitle = ((!(se.Until > 0.0)) ? ((SyncString)string.Empty) : new SyncString(delegate(out string reference, out float period)
			{
				float remainTime = se.GetRemainTime();
				reference = TimedeltaFormatter.Format(remainTime);
				period = remainTime % (float)TimedeltaFormatter.CurrentMinUnit();
			}));
			widgetTooltipControl.Set(text, subtitle, stringBuilder.ToString().Trim(), 500);
			...
			pos.x = Mathf.Max((float)(-UIManager.SafeWidth) * 0.5f + 10f + (float)widgetTooltipControl.Widget.width, pos.x);
	...
	{
		return (_countPerLine <= 0) ? _statusEffectIcons.Count : Mathf.Min(_statusEffectIcons.Count, _countPerLine);
	}

	private int GetIconRowCount()
	{
		return (_countPerLine <= 0) ? 1 : Mathf.CeilToInt((float)_statusEffectIcons.Count / (float)_countPerLine);
	}
}
