...
public class MiniGameDanceNote : MonoBehaviour
{
	...
	private IEnumerator FlyingSequence(float startTime, MiniGameDanceAsset.DanceNoteData data, UIWidget target, Action<float, MiniGameDanceNote, bool> destroyCallback)
	{
		...
		{
			float alpha = (reachAt - cur) / data.TransitionTime;
			base.transform.position = Vector3.Lerp(spawnPos, target.transform.position, 1f - alpha);
			yield return null;
		}
	...
	{
		if ((data.Pattern & MiniGameDanceAsset.DanceNoteData.Type.Dot) != MiniGameDanceAsset.DanceNoteData.Type.None)
		{
		...
		else if ((data.Pattern & MiniGameDanceAsset.DanceNoteData.ArrowPattern) != MiniGameDanceAsset.DanceNoteData.Type.None)
		{
