	...
	public void Show()
	{
		...
		if (farmingEncyclopediaDataList != null)
		{
			...
			Array.Sort(array, delegate(KeyValuePair<string, FarmingEncyclopediaData> d1, KeyValuePair<string, FarmingEncyclopediaData> d2)
			{
				...
				int num4 = itemPriority - itemPriority2;
				if (num4 != 0)
				{
					return num4;
				}
				num4 = d2.Value.CurrentLevel - d1.Value.CurrentLevel;
				if (num4 != 0)
				{
					return num4;
				}
				num4 = d2.Value.CurrentExp - d1.Value.CurrentExp;
				return (num4 != 0) ? num4 : string.CompareOrdinal(d1.Key, d2.Key);
			});
			KeyValuePair<string, FarmingEncyclopediaData>[] array2 = array;
			for (int num3 = 0; num3 < array2.Length; num3++)
			{
				KeyValuePair<string, FarmingEncyclopediaData> keyValuePair = array2[num3];
