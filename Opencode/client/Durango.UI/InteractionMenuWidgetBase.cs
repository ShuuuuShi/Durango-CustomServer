...
public abstract class InteractionMenuWidgetBase : SelectableWidget
{
	...
	public void Set(InteractionMenuData data, InteractionObject target)
	{
		...
		base.transform.localScale = Vector3.one * ((Type != MenuType.Normal) ? InteractionMenuListWidgetBase.MinorScale : InteractionMenuListWidgetBase.MajorScale);
		Name = Data.Name;
		...
		if (Data.GatheringData != null)
		{
			...
			else
			{
				...
				if (flag)
				{
					text2 = "[icon=img_notool]";
				}
			}
			if (Data.GatheringData.IsCritical)
			{
				text = $"[icon={_criticalImageName}]";
		...
		else
		{
			...
			else if (Data.Action == Interaction.RideBalloon)
			{
				...
				if (voucherCount > 0)
				{
					...
					Alpha = _alphaBgDisabled;
					text = $"{voucher.GetIconText()} {voucherCount}";
				}
				else
				{
					Alpha = 1f;
		...
		SetInfoText(text, emphasis);
		SetWaringText(text2, emphasis: false);
		if (Data.Timer != null)
		{
			ProgressGauge.Play(Data.Timer);
			if (NeedSyncTimeLabel(Data.Timer))
			{
				_timeLabel.SetText(new SyncString(delegate(out string reference, out float period)
				{
					if (Data.Timer == null)
					{
						period = 0f;
					...
					else
					{
						...
						reference = GetDurationText(remain);
						if (remain > 10f)
						{
