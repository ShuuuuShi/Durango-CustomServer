...
public class ItemContextPerformance : ItemContextBase
{
	...
	public override void Init()
	{
		...
		{
			UIEventListener uIEventListener = UIEventListener.Get(obj);
			uIEventListener.onClick = (UIEventListener.VoidDelegate)Delegate.Combine(uIEventListener.onClick, new UIEventListener.VoidDelegate(OnClickStatusEffectObject));
		});
	...
	private void FillData(Messages.Pet pet)
	{
		...
		AddKeyValueInfo(T._("생명"), ((int)((stat.Life != null) ? stat.Life.Max() : 0f)/*cast due to constrained. prefix*/).ToString());
		AddKeyValueInfo(T._("활력"), ((int)((stat.Hungry != null) ? stat.Hungry.Max() : 0f)/*cast due to constrained. prefix*/).ToString());
