...
public class TitlePlayerSelectionGroupBase : MonoBehaviour
{
	...
	public void Show(Account account, string serverName, int availableSlotCount, int maxSlotCount, [NotNull] Action<string, int> startWithExistingId, [NotNull] Action<int> startPrlogue, Action<PlayerInfo> deleteClicked)
	{
		...
		bool flag2 = false;
		int num = 0;
		for (int num2 = Mathf.Max(list.Count, availableSlotCount, maxSlotCount); num < num2; num++)
		{
			if (num < list.Count)
			{
				bool wantClicked = list[num].PlayerEntityId == recommendedPlayer.Item1;
				CreateExistingCharacterButton(_scroll.Nodes, list[num], num, startWithExistingId, deleteClicked, wantClicked);
			}
			else if (num < availableSlotCount)
			{
