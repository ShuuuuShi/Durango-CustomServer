...
public class FactionSupportRequestPage : UIWidget
{
	...
	private void RefreshTab()
	{
		...
		for (int i = 0; i < FactionGroup.FactionOrder.Length; i++)
		{
			...
			if (f.EndsAt > 0.0)
			{
				text = new SyncString(delegate(out string reference, out float period)
				{
					double num = f.EndsAt - Connections.Frontend.GetPredictedServerTime();
					if (num > 0.0)
					{
						reference = T._("{0} 남음", TimedeltaFormatter.Format(num, 1, "min"));
			...
			{
				text = new SyncString(delegate(out string text2, out float period)
				{
					SyncString.UpdateRemainTimeMsg(f.SupportRequestAvailableAt, "[icon=icon_timer] {0}", out text2, out period, string.Empty);
				});
			}
			_tabList.Add(icon, text);
			_tabList.SetNotification(count, flag, Type.Normal);
		}
		_tabList.EndLoad();
	}
