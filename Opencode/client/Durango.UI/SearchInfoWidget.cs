...
public class SearchInfoWidget : MonoBehaviour
{
	...
	public void Set(SearchOption option)
	{
		...
		foreach (TagFilterBase elem in option.Tags)
		{
			SearchInfoItemNode component5 = nodes.GetNext().GetComponent<SearchInfoItemNode>();
			component5.Set(elem.GetName(), delegate
			{
				option.Tags.Remove(elem);
				OnClickSearchItem();
			});
		}
		foreach (TagFilterBase elem2 in option.Materials)
		{
			SearchInfoItemNode component6 = nodes.GetNext().GetComponent<SearchInfoItemNode>();
			component6.Set(elem2.GetName(), delegate
			{
				option.Materials.Remove(elem2);
				OnClickSearchItem();
			});
		}
		nodes.EndLoad();
