...
public class SkillCategoryInfoWidget : MonoBehaviour
{
	...
	{
		StatusEffect effect = SkillSystem.FindSkillExpModifier(_category, out var value);
		_eventNotice.gameObject.SetActive(effect != null);
		if (effect != null)
		{
			SyncString text = ((!(effect.Until > 0.0)) ? ((SyncString)string.Empty) : new SyncString(delegate(out string reference, out float period)
			{
			...
			_eventRemainLabel.SetText(text);
