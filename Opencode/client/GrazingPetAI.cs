...
public class GrazingPetAI : StateBasedAI<GrazingPetAI.State>
{
	...
	private IEnumerator OnRoming()
	{
		while (true)
		{
			...
			if (moveSet != null)
			{
				MoveMotionInfo m = moveSet.GetMoveMotion(0f);
				TargetAnimal.SetRotateSpeed(m.rot_speed);
				TargetAnimal.Play(m.motion, loop: true, 0f, PlaybackRate);
				float moveSpeed = m.base_move_speed;
