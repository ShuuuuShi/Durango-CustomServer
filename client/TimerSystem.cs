...
public class TimerSystem : GameSystem<TimerSystem>
{
	...
	private void StopLocalPlayerTimer(InterruptCondition condition)
	{
		...
		for (int count = _timers.Count; i < count; i++)
		{
			if (!(_timers[i].EntityId != playerId) && (_timers[i].InterruptCondition & condition) != InterruptCondition.None)
			{
				_timers[i].Stop();
			}
		}
	...
	{
		if (timer.EntityId == GameManager.PlayerId && !string.IsNullOrEmpty(timer.Subject) && StartSubjectProgress != null)
		{
			StartSubjectProgress(timer.Subject);
		}
	}
	...
	private void TimerFinished(Durango.Logic.Timer.Timer timer)
	{
		...
		if (FinishedSubjectProgress != null)
		{
			FinishedSubjectProgress(timer.Subject, timer.IsInterrupt);
		}
	}
