...
public class CombatGroup : UIBase
{
	...
	private void OnDamage(Damaged damaged)
	{
		...
		if (flag && damage.Value > 0)
		{
			bool flag3 = (damage.Effects & DamageEffects.Critical) != 0;
			float num = ((!flag3) ? 0.5f : 1f);
			float shakeScaleU = UnityEngine.Random.Range(50f, 80f) * num;
			float shakeScaleV = UnityEngine.Random.Range(50f, 80f) * num;
			float value = ((!flag3) ? 0.5f : 1f);
	...
	private void OnBattleActionUsed(BattleAction action)
	{
		...
		if (!(castingBar.Item1 >= castingBar.Item2))
		{
			...
			ActionProgressGauge actionProgressGauge = Durango.Logic.Timer.Timer.Play<ActionProgressGauge>(_actionCastingTimer);
			actionProgressGauge.Set(action.Data.Meta.Icon);
		}
	}
	...
	public virtual void SetBattleView(BattleViewMode mode)
	{
		...
		if (BattleViewChanged != null)
		{
			BattleViewChanged(mode);
		}
	}
	...
	{
		if (BattleView != BattleViewMode.Normal && (!(obj != null) || !NGUITools.IsChild(Singleton<UIManager>.Instance().UIRoot.transform, obj.transform)))
		{
			switch (BattleView)
			{
			case BattleViewMode.Battle:
	...
	private void TouchScreenInMount()
	{
		...
		if (predictedServerTime < artifactComponent.LastProjectile.CoolingUntil)
		{
	...
	private void Start()
	{
		...
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.Attack, delegate(InteractionObject interactionObject)
		{
			SetBattleView(BattleViewMode.Battle);
			GameSystem<CombatSystem>.Instance().SelectTarget(interactionObject.EntityId);
			...
			if (actionSlots != null && actionSlots.Length >= 2)
			{
				...
				{
					UseBattleAction(battleAction.Data.Id);
				}
			}
		});
