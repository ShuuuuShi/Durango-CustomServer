...
public class FactionSummaryPage : KGridScrollView
{
	...
	public void Refresh()
	{
		...
		_factionOrders.Sort(delegate(Durango.Logic.Faction.Faction f1, Durango.Logic.Faction.Faction f2)
		{
			...
			int num2 = Array.IndexOf(FactionGroup.FactionOrder, f1.Type);
			int num3 = Array.IndexOf(FactionGroup.FactionOrder, f2.Type);
			return num2 - num3;
		});
		...
		for (int num = 0; num < _factionOrders.Count; num++)
		{
			Durango.Logic.Faction.Faction faction = _factionOrders[num];
			...
			if (faction.IsAvailable() || !string.IsNullOrEmpty(text))
			{
				...
				PortraitMaterial portraitMaterial = _factionPortraits.Get(_factionOrders[num].Type);
				component.Set(_factionOrders[num], portraitMaterial.Material, portraitMaterial.Uv, text);
	...
	{
		if (TalksClicked != null)
		{
			TalksClicked(comp.Faction.Type);
		}
	}

	private void OnFactionSupportRequestClicked(FactionSummary comp)
	{
		if (SupportRequestClicked != null)
		{
			SupportRequestClicked(comp.Faction.Type);
		}
	}
}
