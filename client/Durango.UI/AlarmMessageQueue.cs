...
public class AlarmMessageQueue : MonoBehaviour, AlarmRewardQueue.IMessageGroup
{
	...
	private void LateRefreshPosition()
	{
		...
		for (int count = _msgLabels.Count; i < count; i++)
		{
			...
			Vector3 vector = (alarmMessageWidget.TargetPosition = Vector3.down * num);
			if (alarmMessageWidget.Index == -1)
			{
				if (i < count - 1 || count == 1)
				{
					transform.localPosition = vector;
					alarmMessageWidget.TargetPosition = vector;
				}
				else
				{
					transform.localPosition = vector + Vector3.down * 10f;
				}
			}
	...
	public void PushMessage(string key, string message, float duration)
	{
		...
		if (num == -1)
		{
			MessageSet item = new MessageSet
			{
				Key = key,
				Text = message,
				Duration = duration
