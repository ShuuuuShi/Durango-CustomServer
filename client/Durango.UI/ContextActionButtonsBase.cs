...
public class ContextActionButtonsBase : MonoBehaviour
{
	...
	private void Init()
	{
		if (!_isInit)
		{
			...
			for (int num2 = 0; num2 < num; num2++)
			{
				...
				contextActionButtonBase.transform.localPosition = _actionSlotPositions[num2];
	...
	public void SetActionCooltime(Interaction key, string argument, double since, double until)
	{
		...
		{
			CooltimeStruct cooltimeStruct = new CooltimeStruct
			{
				Key = key,
				Argument = argument,
				Since = since,
			...
			if (num == -1)
			{
				_cooltimes.Add(cooltimeStruct);
			}
			else
			{
				_cooltimes[num] = cooltimeStruct;
			}
		}
		else if (num != -1)
		{
