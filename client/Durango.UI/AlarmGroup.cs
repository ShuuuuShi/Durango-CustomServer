...
public class AlarmGroup : UIBase
{
		...
		foreach (KeyValuePair<int, List<AlarmRewardQueue.Args>> gatherSkillCategoryUpArg in _gatherSkillCategoryUpArgs)
		{
			...
			if (value.Count > 1)
			{
				RewardAlarm(new AlarmRewardQueue.Args
				{
					...
					Sub = value[0].Sub,
					Icon = string.Empty,
					ExtraIcons = value.Select((AlarmRewardQueue.Args arg) => arg.Icon).ToArray()
				}, RewardEffectType.MultipleSkillCategoryUp);
			}
			else
			{
	...
	private void PvpIslandSystem_Kill(S02PVPKill msg)
	{
		...
		ShowNotify(text2, icon, major, 1.8f, null, null, iconColor);
	}
	...
	{
		return !string.IsNullOrEmpty(msg.MissionId) && msg.MissionId != "sh_sq_01";
	}
	...
	private void ShowRewardAlarm(object effect)
	{
	...
	private void DoCategoryLevelUpRewardEffect(object effect)
	{
		...
		foreach (KeyValuePair<Shared.Skill.Category, int> changedLevel in categoryLevelUpRewardEffect.ChangedLevels)
		{
	...
	{
		ReceiveRewardsPopup receiveRewardsPopup = UIManager.Popup.FindTooltip<ReceiveRewardsPopup>();
		receiveRewardsPopup.ShowRewardInfo(T._("우편 받기"), T._("확인"), "ui_menu_quest_recieve", effectOn: false, _currentRewarded.Reward);
	}
	...
	private void DoArchipelagoRegionRewardsEffect()
	{
		...
		{
			ReceiveRewardsPopup receiveRewardsPopup = UIManager.Popup.FindTooltip<ReceiveRewardsPopup>();
			receiveRewardsPopup.ShowRewardInfo(T._("개척 임무 완료 보상"), T._("확인"), "ui_menu_quest_recieve", effectOn: true, _currentRewarded.Reward);
		}
	...
	{
		ReceiveRewardsPopup receiveRewardsPopup = UIManager.Popup.FindTooltip<ReceiveRewardsPopup>();
		receiveRewardsPopup.ShowRewardInfo(T._("워프 러시 랭킹 보상"), T._("확인"), "ui_menu_quest_recieve", effectOn: true, _currentRewarded.Reward);
	...
	private void DoWarpAccelerationRewardsEffect()
	{
		...
		{
			ReceiveRewardsPopup receiveRewardsPopup = UIManager.Popup.FindTooltip<ReceiveRewardsPopup>();
			receiveRewardsPopup.ShowRewardInfo(T._("워프 가속 보상"), T._("확인"), "ui_warpaccelerator_reward", effectOn: true, _currentRewarded.Reward);
		}
		else
		{
			ReceiveRewardsPopup receiveRewardsPopup2 = UIManager.Popup.FindTooltip<ReceiveRewardsPopup>();
			receiveRewardsPopup2.ShowWarpAcceleratorRewardInfo(T._("워프 가속 보상"), T._("확인"), "ui_warpaccelerator_reward", effectOn: true, _currentRewarded.Reward, warpAcceleratorInfo2.Value);
		}
	}
	...
	private void DoPioneerGradeUpEffect(object effectObject)
	{
		...
		ReceiveRewardsPopup receiveRewardsPopup = UIManager.Popup.FindTooltip<ReceiveRewardsPopup>();
		receiveRewardsPopup.ShowPioneerGradeUp(effect, _currentRewarded.Reward);
	}
	...
	private void DoOpenRewardBoxEffect(object effectObject)
	{
		...
		ReceiveRewardsPopup receiveRewardsPopup = UIManager.Popup.FindTooltip<ReceiveRewardsPopup>();
		receiveRewardsPopup.ShowOpenRewardBox(effect, _currentRewarded.Reward);
	}
	...
	private void DoPetTaskFinishedEffect(object effectObject)
	{
		...
		ReceiveRewardsPopup receiveRewardsPopup = UIManager.Popup.FindTooltip<ReceiveRewardsPopup>();
		receiveRewardsPopup.ShowPetTaskFinished(effect, _currentRewarded.Reward);
	}
	...
	{
		if (GameManager.ClusterMode != Mode.Online && !player.IsLocalPlayer)
		{
	...
	private static string MakeRewardedComment(Rewarded rewarded)
	{
		StringBuilder stringBuilder = new StringBuilder();
		List<string> list = new List<string>();
		object effect = rewarded.Effect;
		if (effect is HuntRewardEffect huntRewardEffect)
		{
			stringBuilder.AppendLine(T._("{0} 사냥에 성공했습니다", huntRewardEffect.TargetAnimal));
		}
		else if (effect is LevelUpEffect levelUpEffect)
		{
			stringBuilder.AppendLine(T._("{0:lv:}{0:-이} 되었습니다", levelUpEffect.Level));
		}
		...
		else if (effect is CategoryLevelUpRewardEffect categoryLevelUpRewardEffect)
		{
			list.Clear();
			foreach (KeyValuePair<Shared.Skill.Category, int> changedLevel in categoryLevelUpRewardEffect.ChangedLevels)
			{
		...
		else if (effect is MissionCompletedEffect missionCompletedEffect)
		{
			stringBuilder.AppendLine(T._("{0}의 임무를 완료했습니다", missionCompletedEffect.FactionType.GetName()));
		}
		else if (effect is DailyMissionCompletedEffect)
		{
		...
		else if (effect is RepairEffect { Result: var result })
		{
			switch (result)
			{
			default:
		...
		int? skillPoints = reward.SkillPoints;
		if (skillPoints.HasValue && skillPoints.GetValueOrDefault() > 0)
		{
