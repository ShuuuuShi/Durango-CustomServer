...
public class WorldMapGroup : UIBase
{
	...
	private void Start()
	{
		...
		{
			Type = ButtonType.WarpBack,
			Icon = "icon_map_warphole",
			Function = WarpBack,
			ValidFunc = delegate
			{
		...
		{
			Type = ButtonType.WarpToPort,
			Icon = "icon_map_port",
			Function = WarpToPort,
			ValidFunc = () => GameSystem<MapSystem>.Instance().GetPOICount(Shared.System.PointOfInterest.Port) > 0
		};
		ButtonInfo buttonInfo4 = new ButtonInfo
		{
			Type = ButtonType.ReturnToClanEstate,
			Icon = "icon_map_clan_building",
			Function = ReturnToClanEstate,
			ValidFunc = () => _estateLicenses.ClanEstate.HasValue
		};
		ButtonInfo buttonInfo5 = new ButtonInfo
		{
			Type = ButtonType.ReturnToUrbanEstate,
			Icon = "icon_map_civilizedisland",
			Function = delegate
			{
				ReturnToEstate(OwnerType.Player);
			},
			ValidFunc = () => _estateLicenses.UrbanEstate.HasValue
		};
		ButtonInfo buttonInfo6 = new ButtonInfo
		{
			Type = ButtonType.ReturnToPersonalEstate,
			Icon = "icon_map_privateisland",
			Function = delegate
			{
				ReturnToEstate(OwnerType.PersonalPlayer);
			},
			ValidFunc = () => _estateLicenses.PersonalEstate.HasValue
		};
		if (GameManager.ClusterMode == Mode.Online)
		{
			_buttonInfos = new ButtonInfo[4][]
			{
				new ButtonInfo[1]
				{
					new ButtonInfo
					{
						Type = ButtonType.ReturnToHome,
						Icon = "icon_house",
				...
				new ButtonInfo[1]
				{
					new ButtonInfo
					{
						Type = ButtonType.SharePosToClan,
						Icon = "icon_map_pinpoint",
						Function = delegate
						{
							SetWorldOpenMode(WorldOpenMode.SharePosToClan);
						},
						ValidFunc = () => PlayerBehavior.LocalPlayer.HasClan
					}
				},
				new ButtonInfo[1]
				{
					new ButtonInfo
					{
						Type = ButtonType.PurchaseMap,
						Icon = null,
						Function = delegate
						{
							PurchaseMap();
						},
						ValidFunc = () => GameSystem<MapSystem>.Instance().CanPurchaseMap && GameSystem<MenuSystem>.Instance().IsEnabled(Durango.Logic.MenuType.Shop)
					}
				}
			};
			switch (GameManager.Region.Role())
			...
			case Role.Urban:
			case Role.Personal:
				_buttonInfos[1] = new ButtonInfo[5] { buttonInfo, buttonInfo3, buttonInfo6, buttonInfo5, buttonInfo4 };
				break;
			case Role.Risky:
				_buttonInfos[1] = new ButtonInfo[4] { buttonInfo2, buttonInfo6, buttonInfo5, buttonInfo4 };
				break;
			case Role.Outpost:
				_buttonInfos[1] = new ButtonInfo[4] { buttonInfo3, buttonInfo6, buttonInfo5, buttonInfo4 };
				break;
			case Role.Safehouse:
				_buttonInfos[1] = new ButtonInfo[1] { buttonInfo2 };
				break;
			}
		}
		else
		...
		uIEventListener.onDoubleClick = delegate
		{
			...
			else
			{
				...
				float num = currentZoomScale + 0.5f;
				num = (float)(int)(num / 0.5f) * 0.5f;
				mapContext.Zoom(num - currentZoomScale, currentTouch.pos);
			}
		};
	...
	{
		if (type.HasValue && type.Value != InfoType.None)
		{
	...
	private static void HideIndicatorsWithoutWarpTargets(MapIndicators inds)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			switch (array[i])
			{
			case IndicatorType.LocalPlayer:
			case IndicatorType.Dock:
			case IndicatorType.Warphole:
			case IndicatorType.POIBiocom:
	...
	{
		OpenMapCost cost = Yaml.Util.Singleton<CostsYaml>.Instance.OpenMap;
		bool free = cost.HasVoucherFromCommodity();
		if (cost.HasVoucher() || free)
		{
			...
			if (free)
			{
				VoucherWithCommodity voucherFromCommodity = cost.GetVoucherFromCommodity();
			...
			string voucherId = cost.GetVoucher().VoucherId;
			messageBox.SetVoucherInfo(voucherId);
			messageBox.Show($"{arg}\n{text}", subText, delegate(bool ok)
			{
				if (ok)
				{
					string voucherId2 = cost.GetVoucherFromCommodity().VoucherId;
					GameSystem<MapSystem>.Instance().PurchaseMap((!free) ? voucherId : voucherId2, _downloadTweenDuration);
				}
	...
	private void ToggleTeleport()
	{
		...
		for (int i = 0; i < _buttonInfos.Length; i++)
		{
			if (KUtility.GetSize(_buttonInfos[i]) != 0)
			{
				ButtonInfo buttonInfo = _buttonInfos[i][0];
				if (buttonInfo.Type == ButtonType.Teleport)
				{
					SelectableButton selectableButton = _actionButtons[i];
					selectableButton.Selected = _isTeleportMode;
					break;
				}
			}
		}
	...
	public static string GetIslandLifeTimeText()
	{
		...
		if (remaingTime > 0.0)
		{
			return (!(remaingTime < 60.0)) ? TimedeltaFormatter.Format(remaingTime, 2, "min") : T._("곧 파괴됨");
		}
		return "-";
	}
}
