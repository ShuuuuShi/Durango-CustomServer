...
public class ClanMembersPage : ClanMenuPage, IUIInitializable
{
	...
	private void SetTabInfos(Clan clan)
	{
		...
		if (_validRole && (_role.GetPermissions() & Permissions.ApproveMember) != Permissions.None)
		{
	...
	private void RefreshTab()
	{
		...
		List<Durango.Logic.Clan.Member> list = ((_currentTab != Tab.Member) ? _memberSorter.Appliers : _memberSorter.Members);
	...
	private void SelectMember(Durango.Logic.Clan.Member member)
	{
		...
		if (_selected != null)
		{
			...
			if (_validRole)
			{
				...
				if ((_role.GetPermissions() & Permissions.PromoteMember) != Permissions.None && flag2)
				{
	...
	private void ShowRoleSelector(Durango.Logic.Clan.Member target)
	{
		...
		foreach (KeyValuePair<int, MemberRole> roleInfo in playerClan.RoleInfos)
		{
			...
			for (int size = KUtility.GetSize(permissions); i < size; i++)
			{
				...
				if ((permissions2 & value.GetPermissions()) != Permissions.None)
				{
