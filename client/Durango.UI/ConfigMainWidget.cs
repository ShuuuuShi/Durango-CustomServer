...
public class ConfigMainWidget : MonoBehaviour
{
	...
	private void RepositionWidgets()
	{
		...
		for (int count = _settingItems.Count; i < count; i++)
		{
			...
			if (settingItem.Type == SettingType.TinyButton)
			{
				...
				settingItem.GameObj.transform.localPosition = new Vector3(20f, -num2);
			...
			else if (settingItem.Type == SettingType.Button)
			{
				...
				if (((selectableButton3 != null) & (x2 < num / 2)) && num3 < num / 2)
				{
					...
					settingItem.GameObj.transform.localPosition = new Vector3(20f, -num2);
					settingItem2.GameObj.transform.localPosition = new Vector3(20 + settingItem.Widget.width + 10, -num2);
					i++;
				}
				else
				{
			...
			else
			{
				...
				if (settingItem.Type == SettingType.Account)
				{
					...
					SelectableButton selectableButton4 = settingItem.SubContent as SelectableButton;
					int x3 = selectableButton4.GetPreferredSize().x;
					selectableButton4.Widget.SetAnchor(settingItem.GameObj, settingItem.Widget.width - x3 - 20, 21, -20, -21);
					float num4 = GetParentPanel().width - 20f;
					if (selectableButton4.gameObject.activeSelf)
					{
						num4 -= (float)(x3 + 20);
					}
	...
	private void SetCategoryWidgets()
	{
		...
		foreach (Setting item in list)
		{
			...
			case SettingType.TextInput:
				...
				{
					settingItem = AddTextInput(valueSetting4);
			...
			case SettingType.CheckBox:
				...
				{
					settingItem = AddCheckBox(valueSetting3);
	...
	{
		bool flag = item.Type == SettingType.CheckBox || item.Type == SettingType.ButtonBox || item.Type == SettingType.Switch;
		UIWidget component = child.GetComponent<UIWidget>();
		float num;
		if (flag)
		{
			num = component.width + 20;
		}
		else
		{
			num = parentWidth - (float)item.Label.width;
			if (item.Type != SettingType.TextLabel)
			{
				num = Mathf.Min(MinItemChildWidth, num);
			}
			component.width = (int)num - 20;
		}
		Vector3 pos = item.Widget.GetPosition(0f, 0.5f) + (parentWidth - num) * Vector3.right;
	...
	private SettingItem AddAccountItem(Setting op)
	{
		...
		UIDragScrollView uIDragScrollView = selectableButton.gameObject.AddMissingComponent<UIDragScrollView>();
	...
	protected virtual SettingItem AddSwitch(ValueSetting op)
	{
		...
		switchWidget.SetValue(value, dispatchEvent: false, immediately: true);
		switchWidget.ValueChanged = delegate(bool value2)
		{
			OnValueChanged(op.Key, value2);
		};
	...
	protected static void RefreshWidget(SettingItem setting)
	{
		...
		case SettingType.Locale:
		{
			ToggleWidget toggleWidget = setting.Contents as ToggleWidget;
			toggleWidget.OnLocalize(setting.Type);
			break;
		}
		case SettingType.Slider:
		{
			SliderWidget sliderWidget = setting.Contents as SliderWidget;
			sliderWidget.SetValue((float)setting.Value);
			break;
		}
		case SettingType.TextInput:
		{
			TextInputOptionWidget textInputOptionWidget = setting.Contents as TextInputOptionWidget;
			textInputOptionWidget.Value = setting.Value as string;
			break;
		}
		case SettingType.TextLabel:
		{
			UILabel uILabel = setting.Contents as UILabel;
			uILabel.text = setting.Value as string;
			break;
		}
		case SettingType.Switch:
		{
	...
	private void DoLocalize(SettingItem setting)
	{
		...
		{
			UILabel uILabel = setting.Contents as UILabel;
			uILabel.text = Platform.Instance.LoginTypeDescription;
			SelectableButton selectableButton2 = setting.SubContent as SelectableButton;
			if (selectableButton2 != null)
			{
				selectableButton2.Text = GetLocalizedText("#config_button_" + setting.Key);
				selectableButton2.gameObject.SetActive(Platform.Instance.IsLoginTypeGuest);
			}
			break;
		}
		case SettingType.Button:
		case SettingType.TinyButton:
		{
			SelectableButton selectableButton = setting.Contents as SelectableButton;
