	...
	private void _MakeTabList()
	{
		...
		case OwnerType.ClanEstate:
		{
			...
			foreach (KeyValuePair<int, MemberRole> roleInfo in _clan.RoleInfos)
			{
				...
				array[num2] = _clan.Members.Count((Durango.Logic.Clan.Member x) => x.RoleId == roleId);
				num2++;
			}
			break;
		}
		...
		bool writable;
		for (; GetAccessRights(_license.Type, num, out nameText, out rights, out writable); num++)
		{
			AccessRightsTab component = _tabScrollView.Nodes.GetNext().GetComponent<AccessRightsTab>();
			string subText = ((array != null && array.Length > num) ? array[num].ToString() : string.Empty);
