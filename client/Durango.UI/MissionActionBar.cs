...
public class MissionActionBar : MonoBehaviour, IUIInitializable
{
	...
	{
		if (_faction != null && _faction.Mission.HasValue && MissionStartClicked != null)
		{
			MissionStartClicked(_faction.Mission.Value);
		}
	}

	private void OnRefreshMission()
	{
		if (_faction != null && _faction.Mission.HasValue && MissionRefreshClicked != null)
		{
			MissionRefreshClicked(_faction.Mission.Value);
		}
	}

	private void OnCancelMission()
	{
		if (_faction != null && _faction.Mission.HasValue && MissionCancelClicked != null)
		{
			MissionCancelClicked(_faction.Mission.Value);
		}
	}

	private void OnResetMissionCooltime()
	{
		if (_faction != null && MissionResetCooltimeClicked != null)
		{
			MissionResetCooltimeClicked(_faction.Type);
		}
	}

	private void OnMissionDetail()
	{
		if (_faction != null && _faction.Mission.HasValue && MissionDetailClicked != null)
		{
			MissionDetailClicked(_faction.Mission.Value);
		}
	}
}
