...
public class SkillListWidget : MonoBehaviour
{
	...
	public void Set(Shared.Skill.Category cat, string group)
	{
		...
		for (int i = 0; i < skills.Count; i++)
		{
			...
			if (num == -1)
			{
				obj = new Group();
				obj.Name = value;
				obj.Skills = new List<Bundle>();
				_skillGroups.Add(obj);
			}
			else
			{
				obj = _skillGroups[num];
			}
			obj.Skills.Add(skills[i]);
		}
		...
		for (int k = 0; k < _skillGroups.Count; k++)
		{
			...
			Group obj2 = _skillGroups[k];
			component.Set(obj2);
			if (num2 == -1 && group == obj2.Name)
			{
				num2 = k;
			}
		}
	...
	private void OnSelectSkillGroup(Group group)
	{
		...
		if (group.Skill == null)
		{
			...
			{
				SkillGroupSelected(group);
			}
		}
		else if (SkillSelected != null)
		{
			SkillSelected(group.Skill);
		}
	}
}
