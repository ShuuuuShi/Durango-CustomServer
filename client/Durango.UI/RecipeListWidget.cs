...
public class RecipeListWidget : MonoBehaviour, IUIInitializable
{
	public class SubList
	{
		...
		{
			RecipeSubListWidget.Data item2 = new RecipeSubListWidget.Data
			{
				Item = item
			};
			_list.Add(item2);
		...
		{
			return (!string.IsNullOrEmpty(searchText)) ? _list.Where((RecipeSubListWidget.Data data) => PredicateSearchFilter(data, searchText)) : _list;
		}
	...
	private void RecipeSubListWidget_RecipeClicked(RecipeItemWidget widget, bool inFavorites)
	{
		if (widget != null)
		{
			RefreshSelection((!_toggleRecipeSelection || !SelectedRecipe.HasValue || !(SelectedRecipe.Value.Id == widget.Item.Id)) ? new SelectInfo?(new SelectInfo(widget.Item, inFavorites)) : ((SelectInfo?)null));
		}
	}
}
