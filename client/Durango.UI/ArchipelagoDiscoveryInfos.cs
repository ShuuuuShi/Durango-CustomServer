...
public class ArchipelagoDiscoveryInfos : MonoBehaviour
{
	...
	public void Set([CanBeNull] List<RegionTemplate> templates, ArchipelagoRoute archipelagoRoute, bool isUnstableFactorVisible)
	{
		...
		GameSystem<MapSystem>.Instance().GetDiscoveryInfos(templates.Select((RegionTemplate template) => template.Id).ToList(), delegate(Messages.DiscoveryInfo[] array)
		{
			...
			{
				Messages.DiscoveryInfo discoveryInfo2 = array[j];
