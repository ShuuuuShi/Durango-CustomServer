			...
			for (int num3 = _makingNotes.Count - 1; num3 >= 0; num3--)
			{
				...
				if (note.Tick + _music.Division <= num2)
				{
					Note note2 = new Note
					{
						...
						Midi = note.Midi,
						Tick = note.Tick + _music.Division,
						Volume = 0f,
						On = false
					};
					FinishMakeNote(note2);
				}
			}
		...
		if (press)
		{
			if (!(_currentGuideLineTimer < 0f))
			{
				Note note = new Note
				{
					Channel = 0,
					Midi = midi,
					Tick = _music.TimerToTick(_currentGuideLineTimer),
					Volume = 1f,
					On = true
				};
				bool flag = _makingNotes.Count > 0 || _isSpacePress;
				BeginMakeNote(note, flag ? DuplicatedNoteProcess.Replace : DuplicatedNoteProcess.Select);
			}
		}
		else
		{
