	...
	private void SetInfos()
	{
		if (_clan != null)
		{
			...
			if (flag)
			{
				...
				{
					double num2 = _clan.RenameableUntil - Connections.Frontend.GetPredictedServerTime();
					string text2 = TimedeltaFormatter.Format(num2);
					text = T._("[icon=icon_make_alert]\n{0} 동안 수정 가능", text2);
					period = (float)num2 % 1f;
				}));
			}
		}
		...
		{
			for (int num = 0; num < _editableMarks.Length; num++)
			{
				_editableMarks[num].gameObject.SetActive(_hasEditPermission);
			}
		}
	}
