...
public class MusicEditorGroup : UIBase
{
	...
	private void Start()
	{
		...
		Durango.Utils.Singleton<ArtifactManager>.Instance().StateChangedWithPrev += delegate(Artifact artifact, ArtifactState prev)
		{
			...
			if (flag || flag2)
			{
				...
				if (flag2)
				{
					...
					{
						chatBubbleGroup.Show(artifact.ChatableBase, null, null, "icon_hormony", Color.black, ChatBubble.TargetPivot.Up, Vector3.zero, alwaysInScreen: false, (float)(num - predictedServerTime));
						return;
					}
				}
		...
		GameSystem<InteractionSystem>.Instance().AddInteractionHandler(Interaction.HostConcert, delegate(InteractionObject obj)
		{
			...
			{
				MusicManager.HostConcert(targetComponent.GetPropKey());
				ConcertPopup concertPopup = UIManager.Popup.Tooltip<ConcertPopup>();
				concertPopup.SetReserve(targetComponent.EntityId);
			}
		});
	...
	public static RadioLink MakeMusicExport(string sharedSheetId, string musicName)
	{
		...
		{
			Text = T._("<em>{0}</em> 악보를 공유합니다.", musicName),
			Link = $"icon=icon_music_small, color=FF7E29, link=ui://Music/Import/{sharedSheetId}"
		};
	}
	...
	private IEnumerator CoConcertTest(List<KeyValuePair<MusicId, Messages.Music>> musics)
	{
		...
		foreach (KeyValuePair<MusicId, Messages.Music> m in musics)
		{
			selector.ResetArguments();
			selector.SetTitle(m.Value.Name);
			string[] array = texts;
			...
			selector.SetSelected(delegate(int index)
			{
				...
				list.Add(new KeyValuePair<string, Messages.Music>(instrument.Id, m.Value));
			});
			selector.Show();
