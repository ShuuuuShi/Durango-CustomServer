...
public class InventoryContainerBase : MonoBehaviour, IUIInitializable
{
	...
	void IUIInitializable.Init()
	{
		...
		{
			UIManager.Popup.Tooltip<TextInputPopup>().Show(delegate(string key)
			{
				GameSystem<InventorySystem>.Instance().AddWarehouseCategory(key);
			}, T._("새로 추가할 탭을 적어주세요"));
		});
		...
		foreach (string text in array)
		{
			...
			component.gameObject.FindComponent<UISprite>("Icon").spriteName = string.Format("bag_category_{0}", text.Replace("/", "_"));
			string id = text;
			component.Clicked = (Action)Delegate.Combine(component.Clicked, (Action)delegate
			{
				int num2 = list.IndexOf(id);
				if (num2 >= 0)
				{
					_categoryTabList.Nodes.Get<SelectableWidget>(_selectedCategoryTabIndex).Selected = false;
					_categoryTabList.Nodes.Get<SelectableWidget>(num2).Selected = true;
					_selectedCategoryTabIndex = num2;
	...
	private void ShowItemInfo(ItemIconWidget itemIcon)
	{
		...
		if (itemIcon.IconMode != ItemIconWidget.Mode.Enabled && _selectedTab == 0 && _other != null)
		{
	...
	private void ItemAction(UseType type, IList<ItemData> items)
	{
		if (KUtility.GetSize(items) != 0)
		{
			...
			case UseType.Grazing:
			{
				...
				stringBuilder.Append("pet_grazing");
	...
	private static void DisplayPutInMessage(IList<ItemData> movableItems, int originalItemsCount)
	{
		...
		if (trackingInventory.State != Durango.Logic.Item.Inventory.InventoryState.Invalid && !trackingInventory.OnlyTakeOut && originalItemsCount != 0 && Durango.Logic.Item.Inventory.CheckEnableUseType(movableItems, UseType.PutIn))
		{
	...
	private static void DropToSpecificTile(DumpItems dumpItems, string warningMessage)
	{
		...
		BuildLocator.Arguments args = ((blueprint != null) ? BuildLocator.Arguments.MakeFrom(blueprint) : new BuildLocator.Arguments
		{
			Size = Point2.one,
			Exterior = true
		});
	...
	private static bool DropToEstate(EstateInfo estate, string warningMessage, Action drop)
	{
		...
		case OwnerType.ClanWarphole:
			...
			{
				MessageBox messageBox = UIManager.MessageBox;
				messageBox.Show(T._("<em>{0}</em> 부족의 영토에 아이템을 버리시겠습니까?", (clan != null) ? clan.Name : string.Empty), warningMessage, delegate(bool ok)
				{
					if (ok)
					{
						drop();
