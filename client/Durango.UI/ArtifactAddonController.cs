...
public class ArtifactAddonController : MonoBehaviour
{
	...
	private int CurrentFloor()
	{
		...
		if (artifact != null)
		{
			...
			if (!(artifact.HasRoof.Value ?? true))
			{
				a--;
			}
			a = Mathf.Max(0, a);
	...
	private void ConfirmAddon()
	{
		...
		if (PrevIndex == -1)
		{
			if (AddonPlaced != null)
			{
				AddonPlaced(SelectedAddon.Item);
			}
			if (modularAddon != null)
			{
				foreach (ModelComponent.IModel selectedPlaceWall in _selectedPlaceWalls)
				...
				if (OnAddonRemove != null)
				{
					OnAddonRemove(modularAddon.Item);
				}
			}
			else
			{
		...
		{
			if (PrevIndex != SelectIndex && AddonMoved != null)
			{
				AddonMoved(PrevIndex, SelectIndex);
			}
			UnselectAddon();
		}
	}
	...
	private void RemoveAddon(int index)
	{
		...
		if (itemData != null && OnAddonRemove != null)
		{
			OnAddonRemove(itemData);
		}
	}
	...
	private void OnRemovedAddon()
	{
		...
		_unselectItem = null;
		if (unselectItem != null && AddonRemoved != null)
		{
			AddonRemoved(unselectItem);
		}
		UnselectAddon();
	}
}
