	...
	public void Set(Durango.Logic.Shop.Purchase purchase)
	{
		...
		else
		{
			...
			else
			{
				...
				text = ((commodity == null || !commodity.IsProduct) ? T._("받기") : T._("개봉"));
			}
			if (purchase.ExpiresAt > 0.0)
			{
				_subText.gameObject.SetActive(value: true);
				_subText.SetText(new SyncString(delegate(out string text2, out float period)
				{
					SyncString.UpdateRemainTimeMsg(purchase.ExpiresAt, T._("{0} 내 수령 필요"), out text2, out period, string.Empty);
				}));
			}
			else
			{
