...
public class EstatePage : MonoBehaviour, IUIInitializable
{
	...
	void IUIInitializable.Init()
	{
		...
		_emptyPersonalEstatePage.DeclareButtonClicked += delegate
		{
			...
			estateGridGroup.Open(OwnerType.PersonalPlayer, _largestSize);
		};
		_emptyUrbanEstatePage.DeclareButtonClicked += delegate
		{
			EstateGridGroup estateGridGroup = UIManager.FindScript<EstateGridGroup>();
			estateGridGroup.Open(OwnerType.Player, _largestSize);
		};
	...
	private void EstateRemove()
	{
		...
		case OwnerType.PersonalPlayer:
			onOkCancel = delegate(bool ok)
			{
				if (ok)
				{
					UIManager.MessageBox.Show(T._("모든 건물의 소유권을 잃습니다. 정말 포기하시겠습니까?"), string.Format("{0}\n{1}", T._("<alert_icon/> 사유지 포기 후 다시 확장할 때 비용이 들지 않습니다."), T._("<alert><alert_icon/> 즉시 소유권을 잃게 되므로 건물을 미리 포장하는 것이 좋습니다.</alert>")), delegate(bool flag)
					{
						if (flag)
						{
							shrinkAction();
		...
		case OwnerType.ClanEstate:
			onOkCancel = delegate(bool ok)
			{
				if (ok)
				{
					UIManager.MessageBox.Show(T._("모든 건물의 소유권을 잃습니다. 정말 포기하시겠습니까?"), string.Format("{0}\n{1}", T._("<alert_icon/> 부족 영토 포기 후 다시 확장할 때 비용이 들지 않습니다."), T._("<alert><alert_icon/> 즉시 소유권을 잃게 되므로 건물을 미리 포장하는 것이 좋습니다.</alert>")), delegate(bool flag)
					{
						if (flag)
						{
							shrinkAction();
	...
	private void EstateExtend()
	{
		...
		EstateGroup.ShowExtendEstatePopup(_license.Value, delegate
		{
			...
			if (license2.HasValue)
			{
				...
				EstateSystem.ExtendEstate(_license.Value.EstateId, extendingCost * _license.Value.Size * 7, delegate(EstateLicense value)
				{
					_parent.NotifyEstateLicenseChanged(value.Type, value);
				});
			}
		});
	}
