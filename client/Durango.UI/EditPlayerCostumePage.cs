...
public class EditPlayerCostumePage : MonoBehaviour, IEditPlayerDisplayPage
{
	...
	public void Initialize(EditPlayerDisplayProxy display)
	{
		...
		_subtabView.Nodes.Init(delegate(GameObject gameObject)
		{
		...
		for (int num = 0; num < _mainTabs.Length; num++)
		{
			Selectable obj = _mainTabs[num];
			obj.Clicked = (Action)Delegate.Combine(obj.Clicked, new Action(OnClickMainTab));
		}
