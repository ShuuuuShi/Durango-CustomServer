...
public class EstateGridGroup : UIBase
{
	...
	{
		EstateLicense license = _expandEstate.License;
		if (!license.Deposit.HasValue || !license.DepositRunsOutAt.HasValue)
		{
			return;
		}
		OwnerType type = license.Type;
		double predictedServerTime = Connections.Frontend.GetPredictedServerTime();
		long num = Yaml.Util.Singleton<CostsYaml>.Instance.Estate.GetExtendingCost(license.Type, license.Size) * license.Size;
		double num2 = (double)license.Deposit.Value.Item1 - (double)num * (predictedServerTime - license.Deposit.Value.Item2) / 86400.0;
		long num3 = Yaml.Util.Singleton<CostsYaml>.Instance.Estate.GetExtendingCost(license.Type, license.Size + 1) * (license.Size + 1);
		if (num3 <= 0)
		{
			return;
		}
		...
		case OwnerType.Player:
			mainText = T._("사유지를 {0}칸으로 확장하시겠습니까?", license.Size + 1);
			text = T._("[icon=icon_make_alert] 확장 시 사유지 유지비가 늘어납니다.");
			break;
		case OwnerType.ClanEstate:
			mainText = T._("부족 영토를 {0}칸으로 확장하시겠습니까?", license.Size + 1);
		...
		long expandingCost = Yaml.Util.Singleton<CostsYaml>.Instance.Estate.GetExpandingCost(license.Type, license.Size);
		string confirm = ((license.Size >= _largestSize) ? Durango.Logic.Item.Inventory.ToCurrencyButtonText(T._("확장"), expandingCost, Currency.TStone) : T._("확장 무료!"));
		bool flag = false;
		if (num4 < 86400.0)
		{
		...
		{
			double num5 = license.DepositRunsOutAt.Value - (predictedServerTime + num4);
			if (num5 > 0.0)
			{
		...
		messageBox.Show(mainText, text, delegate(bool ok)
		{
			if (ok)
			{
				EstateSystem.ExpandEstate(id, unit, delegate(EstateLicense license2)
				{
		...
		else
		{
			...
			case OwnerType.Player:
				onOkCancel = delegate(bool ok)
				{
					if (ok)
					{
						UIManager.MessageBox.Show(T._("모든 건물의 소유권을 잃습니다. 정말 포기하시겠습니까?"), string.Format("{0}\n{1}", T._("[icon=icon_make_alert] 사유지 포기 후 다시 확장할 때 비용이 들지 않습니다."), T._("<alert>[icon=icon_make_alert] 즉시 소유권을 잃게 되므로 건물을 미리 포장하는 것이 좋습니다.</alert>")), delegate(bool flag)
						{
							if (flag)
							{
								shrinkAction();
			...
			case OwnerType.ClanEstate:
				onOkCancel = delegate(bool ok)
				{
					if (ok)
					{
						UIManager.MessageBox.Show(T._("모든 건물의 소유권을 잃습니다. 정말 포기하시겠습니까?"), string.Format("{0}\n{1}", T._("[icon=icon_make_alert] 부족 영토 포기 후 다시 확장할 때 비용이 들지 않습니다."), T._("<alert>[icon=icon_make_alert] 즉시 소유권을 잃게 되므로 건물을 미리 포장하는 것이 좋습니다.</alert>")), delegate(bool flag)
						{
							if (flag)
							{
								shrinkAction();
	...
	private void OnDeclareEstateClick(Point2 pos)
	{
		...
		Durango.Utils.Singleton<PlayerController>.Instance().MoveToPosition(Durango.Terrain.Util.TilePositionToClientPosition((unit.ToVector2() + Vector2.one * 0.5f) * 4f), delegate
		{
			...
			{
				if (type != OwnerType.PersonalPlayer && type != OwnerType.Player)
				{
					if (type == OwnerType.ClanEstate)
					{
