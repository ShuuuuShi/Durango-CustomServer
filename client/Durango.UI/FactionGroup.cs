...
public class FactionGroup : UIBase, INotificationable
{
	...
	public static string FactionTalksToString(Talk talk)
	{
		...
		{
			string arg = ((talk.Target.Value != Shared.Faction.Messenger.Player) ? ((string)SingletonDict<Shared.Faction.Messenger, Yaml.Messenger>.Get(talk.Target.Value).Name) : PlayerBehavior.LocalPlayer.PlayerName);
