...
public class FactionSupportRequestList : UIWidget
{
	...
	public void Refresh()
	{
		...
		_factionTitleLabel.SetText(new SyncString(delegate(out string text, out float period)
		{
			...
			double num = GameSystem<FactionSystem>.Instance().SupportRequestsEndAt - Connections.Frontend.GetPredictedServerTime();
			if (num > 0.0)
			{
				text += T._(" [size=19]{0} 뒤 목록 갱신 <help>{1}</help>", TimedeltaFormatter.Format(num), T._("갱신시, 지원 목록이 바뀌고 요청 횟수가 초기화 됩니다."));
