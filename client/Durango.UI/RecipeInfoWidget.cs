...
public class RecipeInfoWidget : MonoBehaviour, IUIInitializable
{
	...
	private void ShowRecipeDetailInfo(Crafting.Recipe recipe, bool reset)
	{
		...
		SlotStruct toolInfo = ((!recipe.HasRequiredTool) ? default(SlotStruct) : new SlotStruct
		{
			Name = Durango.Logic.Item.Util.LocalizedTagRequiredMsg(recipe.AllowedTool, showLevel: false),
			Count = (hasTool ? 1 : 0),
			RequiredCount = 1,
			RequiredLevel = ((recipe.AllowedTool.Count > 0) ? recipe.AllowedTool[0].Level : 0),
			Tags = recipe.AllowedTool
		});
		SetNonResiable();
	...
	private void ShowBlueprintDetailInfo(Building.Blueprint blueprint, bool reset)
	{
		...
		SlotStruct toolInfo = ((blueprint.AllowedToolTag.Count <= 0) ? default(SlotStruct) : new SlotStruct
		{
			Name = Durango.Logic.Item.Util.LocalizedTagRequiredMsg(blueprint.AllowedToolTag, showLevel: false),
			Count = (hasTool ? 1 : 0),
			RequiredCount = 1,
			RequiredLevel = ((blueprint.AllowedToolTag.Count > 0) ? blueprint.AllowedToolTag[0].Level : 0),
			Tags = blueprint.AllowedToolTag
		});
		SetMaterials(recipeSlotInfos, toolInfo);
	...
	{
		if (BuildSizeChanged != null)
		{
			BuildSizeChanged(_xSelector.Value, _ySelector.Value);
		}
	}
}
