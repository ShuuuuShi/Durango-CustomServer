...
public class ScreenCaptureGroup : UIBase
{
	...
	{
		return script != base.VisibleController && script.GetComponent<AlarmGroup>() == null && script.GetComponent<MessageBox>() == null;
	}
	...
	private void SetActivePersonalMapsButton(bool active)
	{
		if (active)
		{
			...
			{
				_textPersonalMapsTooltip.text = T._("개인섬에서 가능");
			}
	...
	private void RequestCaptureScreenPermissionResult(bool granted)
	{
		...
		{
			OnPreScreenCapture();
			ScreenCapture.Capture(new ScreenCapture.CaptureOption
			{
				Effect = _effect,
				Logo = true,
				NoUI = true,
				OnResult = SetInstagramShot
			});
	...
	private void RequestCaptureEstatePermissionResult(bool granted)
	{
		if (granted)
		{
			if (!TryGetPersonalIslandEstateArea(out var minTile, out var maxTile))
			{
				return;
			}
			OnPreScreenCapture();
			...
			Singleton<PersonalMaps>.Instance().Capture(minTile, maxTile, delegate(float? percentage)
			{
			...
			}, delegate(MemoryStream memoryStream)
			{
				...
				if (memoryStream != null)
				{
					...
					ScreenshotManager.SaveImage(memoryStream);
	...
	private static bool HasPersonalIslandEstateArea()
	{
		if (GameSystem<EstateSystem>.Instance().PersonalIslandInfo.HasValue)
		{
			...
			return estateCellArea.Item1 != Point2.zero || estateCellArea.Item2 != Point2.zero;
		}
		return false;
	}
	...
	private void CaptureScreenForEditor(bool noUI)
	{
		if (Application.isEditor)
		{
			...
			{
				NoUI = noUI,
				OnResult = delegate(Texture2D origin)
				{
