...
public class PortraitMenuList : MenuListWidgetBase, IMenuList
{
	...
	public void Refresh()
	{
		...
		widgets.Clear();
		for (int num = 0; num < _menuList.Count; num++)
		{
			widgets.Add(_menuList[num].Widget);
		}
		int num2 = _menuMinimumWidth;
		for (int num3 = 0; num3 < _menuList.Count; num3++)
		{
			num2 = Mathf.Max(num2, _menuList[num3].GetPreferredSize());
		}
		_scrollContainer.rightAnchor.absolute = num2;
		for (int num4 = 0; num4 < _menuList.Count; num4++)
		{
			_menuList[num4].Widget.width = num2;
		}
