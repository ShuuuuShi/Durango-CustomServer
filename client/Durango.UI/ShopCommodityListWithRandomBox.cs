...
public class ShopCommodityListWithRandomBox : ShopCommodityListBase, IScreenResizeReceiver
{
	...
	protected override void OnInit()
	{
		...
		{
			Selectable component = obj.GetComponent<Selectable>();
			component.Clicked = (Action)Delegate.Combine(component.Clicked, new Action(ItemContentClicked));
		});
		_motionContentsScroll.Nodes.Init(delegate(GameObject obj)
		{
			Selectable component = obj.GetComponent<Selectable>();
			component.Clicked = (Action)Delegate.Combine(component.Clicked, new Action(MotionContentClicked));
		});
	...
	private void SelectItemContent(int index, bool showTooltip)
	{
		...
		for (int num = 0; num < _itemContentsScroll.Nodes.Count; num++)
		{
			_itemContentsScroll.Nodes[num].GetComponent<Selectable>().Selected = num == index;
		}
	}
	...
	private void SelectMotionContent(int index)
	{
		...
		Durango.Logic.Social.Motion motion = ((text != null) ? GameSystem<SocialSystem>.Instance().Emotional.GetMotion(text) : null);
		if (motion == null)
		{
			_modelViewer.gameObject.SetActive(value: false);
		}
		else
		{
			...
			display.Equip = null;
			string motion2 = motion.MotionNames.Random();
			_modelViewer.SetPlayerModel(PlayerBehavior.LocalPlayer.IsMale, display, new UIModelViewer.Arguments
			{
				...
				Loaded = delegate(GameObject obj)
				{
					...
					{
						component.PlayMotionForcely(motion2, 1f, immediately: true);
					}
				}
			});
		}
		for (int num = 0; num < _motionContentsScroll.Nodes.Count; num++)
		{
			_motionContentsScroll.Nodes[num].GetComponent<Selectable>().Selected = num == index;
		}
	}
