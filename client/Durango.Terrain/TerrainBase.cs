...
public abstract class TerrainBase : Singleton<TerrainBase>
{
	...
	private void Update()
	{
		if (IsPlayerInitialized)
		{
			...
			if (ChunkPool.UpdateChunks(currentPosition) && CenterChunkChanged != null)
			{
				CenterChunkChanged();
			}
			ReloadFailedChunks();
	...
	private void RequestChunk(Point2 coord, ChunkData chunkData, bool disableCache)
	{
		...
		Http.Request(url, delegate(byte[] bytes, HTTPResponse response)
		{
			...
			else
			{
				...
				if (terrainChunkBase != null && ChunkLoaded != null)
				{
					ChunkLoaded(terrainChunkBase);
				}
			}
		}, disableCache);
	}
	...
	protected void OnLoadingChunksFinished()
	{
		...
		if (LoadingChunksFinished != null)
		{
			LoadingChunksFinished();
		}
	}
