...
public static class Util
{
	...
	public static bool SetPreview(this ItemData item, [CanBeNull] UIModelViewer viewer, Action<GameObject> loaded = null)
	{
		...
		if (item.HasTag("dye_medicine"))
		{
			if (viewer != null)
			{
				ArtifactDisplay display = new ArtifactDisplay
				{
					Condition = Shared.Building.Condition.Normal,
					Parts = new Dictionary<string, string> { { "common", "dye_02" } },
					Decorations = new Dictionary<string, Pair<string, string>> { 
					{
						"__workbench__",
