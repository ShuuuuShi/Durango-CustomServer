...
public class BuildSlotContainer : SlotContainer<BuildSlotInfo>
{
	...
	public void GetEstimation(Action<BuildEstimation?> onResult)
	{
		if (onResult != null)
		{
			EstimateBuild info = new EstimateBuild
			{
				EntityId = Artifact.EntityId,
				Tile = Artifact.WorldTile,
				ToolId = GetToolItemId(),
				Materials = ((!Blueprint.IsRemodeling) ? CreateFirstMaterialsDictionary(canFinished: false) : null)
			};
	...
	{
		EstimateRemodeling info = new EstimateRemodeling
		{
			EntityId = Artifact.EntityId,
			Tile = Artifact.WorldTile,
			ToolId = GetToolItemId(),
			Materials = CreateFirstMaterialsDictionary(canFinished: false)
		};
