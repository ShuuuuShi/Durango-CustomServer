...
public class InventorySystem : GameSystem<InventorySystem>
{
	...
	{
		if (PlayerItemExpired != null)
		{
			PlayerItemExpired(msg);
		}
	...
	private void ReceiveInventoryUpdated(InventoryUpdated msg, PacketHeader header)
	{
		...
		else if (msg.EntityId == _trackingInventory.OwnerId)
		{
			...
			if (TrackingInventoryUpdated != null)
			{
				TrackingInventoryUpdated();
			}
		}
	}
	...
	{
		ItemData itemData = AddInventoryItem(_playerInventory, msgItem, out var isNew);
		if (itemData == null)
		{
			return;
		}
	...
	private void UpdatePlayerWallet(Wallet? wallet)
	{
		...
		if (wallet2.HasValue)
		{
			for (int i = 0; i < array.Length; i++)
			{
				...
				if (num != balance && BalanceChanged != null)
				{
					BalanceChanged(array[i], num, num - balance);
				}
			}
		}
	...
	public void UseItem(ItemData item, bool playerAccepted = false, Action onSuccess = null)
	{
		...
		}).On(delegate(StartTimer msg, PacketHeader header)
		{
			...
			if (UseItemStarted != null)
			{
				UseItemStarted(item, msg);
			}
			if (UseItemSucceed != null)
			{
				UseItemSucceed(item);
			}
		}).On<OK>(delegate
