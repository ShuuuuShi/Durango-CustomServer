...
public class PrologueConnectionHook : MonoBehaviour, IConnectionHook
{
	...
	private void MakePlayerDamage(UseBattleAction action)
	{
		...
		if (playerActionAttackInfo != null)
		{
			...
			{
				Damaged msg = new Damaged
				{
					AttackerId = GameManager.PlayerId,
					VictimId = action.TargetEntityId,
					Damage = new Damage
					{
					...
					EventAt = Connections.Frontend.GetBufferedServerTime() + (double)playerActionAttackInfo.AttackTime
				};
