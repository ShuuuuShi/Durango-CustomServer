...
public class ArtifactManager : Durango.Utils.Singleton<ArtifactManager>
{
	...
	private void Start()
	{
		...
		{
			Artifact artifact = Find(msg.PropKey.EntityId);
			if (!(artifact == null))
			{
				ArtifactState artifactState = artifact.ArtifactState;
				artifactState.Bandstand = msg;
				UpdateArtifactState(artifact, artifactState, header.Time);
			}
		});
		...
		{
			Artifact artifact = Find(msg.EntityId);
			if ((bool)artifact)
			{
	...
	private void UpdateArtifactState(Artifact artifact, ArtifactState state, double eventTime)
	{
		if (!(artifact == null))
		{
			...
			{
				StateChanged(artifact);
			}
			if (StateChangedWithPrev != null)
			{
				StateChangedWithPrev(artifact, artifactState);
			}
		}
	}
