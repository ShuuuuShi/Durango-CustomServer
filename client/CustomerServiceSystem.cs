...
public class CustomerServiceSystem : GameSystem<CustomerServiceSystem>
{
	...
	private void OnLastAnswerTime(byte[] bytes, HTTPResponse response)
	{
		if (bytes != null)
		{
			...
			float num = Preferences.GetFloat("last_read_cs_time", 0f, Preferences.Level.User);
			double? lastAnswerTime = _lastAnswerTime;
			HasUnreadAnswer = lastAnswerTime.HasValue && lastAnswerTime.GetValueOrDefault() > (double)num;
		}
	}
	...
	private void OnHasUnreadAnswerUpdated()
	{
		...
		if (HasUnreadAnswerUpdated != null)
		{
			HasUnreadAnswerUpdated();
		}
	}
