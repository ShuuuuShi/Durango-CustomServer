...
public void GetAllySlots()
	...
	public static void SetMemberRoleGrades(IList<int> order, Action<bool> onResult)
	{
		...
		SetMemberRoleGrades msg = new SetMemberRoleGrades
		{
			RoleOrders = new RoleOrder[order.Count]
		};
	...
	{
		SetMemberRoleInfo msg = new SetMemberRoleInfo
		{
			RoleId = role.Id,
			Info = role
		};
	...
	{
		RemoveMemberRole msg = new RemoveMemberRole
		{
			RoleId = roleId,
			MoveToRoleId = moveToId
		};
		Connections.Frontend.Send(msg).All(delegate(Packet packet)
		{
			bool flag = Packet.IsSuccess(packet);
