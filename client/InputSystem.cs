...
public class InputSystem : GameSystem<InputSystem>
{
	...
	public bool MoveLock
	{
		...
		set
		{
			SetMoveLockTimer((!value) ? ((float?)null) : new float?(0f));
		}
	}
	...
	private void SetMoveLockTimer(float? duration)
	{
		...
		{
			_moveLockedAt = ((!duration.HasValue) ? ((float?)null) : new float?(Time.time + duration.GetValueOrDefault()));
		}
		else
		{
			_moveLockedAt = 0f;
