...
public class GatheringSystem : GameSystem<GatheringSystem>
{
	...
	private PredictTimer _collectTimer;
	...
	private void OnCollected(Collected msg, PacketHeader header)
	{
		...
		if (!string.IsNullOrEmpty(obj.Id) && ItemCollected != null)
		{
			ItemCollected(obj);
		}
		CurrentGatheringData = null;
		if (msg.RanOut && TargetRunOut != null)
		{
			TargetRunOut();
		}
	...
	private void PlayCollectedAlarm(Collected m)
	{
		...
		AlarmRewardQueue.Args args = new AlarmRewardQueue.Args
		{
			Sub = Durango.Logic.Item.Util.ItemQualityString(item),
		...
		case Result.BigFailure:
			args.Main = T._("채집 실패");
			args.Sub = T._("성공 확률 {0:P0}", m.ActionInfo.SuccessRatio);
			args.Icon = ((CurrentGatheringData != null) ? CurrentGatheringData.Icon : "icon_question");
			eventName = "UI_Gathering_Fail_01";
			type = AlarmGroup.RewardEffectType.CollectFail;
			break;
		case Result.Failure:
			args.Main = T._("{1:lv:} <em>{0}</em> 채집 실수", item.Name, item.Level);
			eventName = "UI_Gathering_Fail_01";
			type = AlarmGroup.RewardEffectType.Collect;
			break;
		case Result.Success:
			args.Main = T._("{1:lv:} <em>{0}</em> 채집", item.Name, item.Level);
			eventName = "UI_Gathering_Success_01";
			type = AlarmGroup.RewardEffectType.Collect;
			break;
		case Result.GreatSuccess:
			args.Main = T._("{1:lv:} <em>{0}</em> 채집 <em>대성공!</em>", item.Name, item.Level);
			eventName = "UI_Gathering_Great_01";
			type = AlarmGroup.RewardEffectType.Collect;
			break;
		}
		UIManager.Alarm.RewardAlarm(args, type);
	...
	public void UpdateCollectibleDisplay(string entityId, CollectibleDisplay? msg)
	{
		...
		{
			if (_hasCollectiblePermissionEntities.Add(entityId) && CollectiblePermissionChanged != null)
			{
				CollectiblePermissionChanged(entityId, arg2: true);
			}
		}
		else if (_hasCollectiblePermissionEntities.Remove(entityId) && CollectiblePermissionChanged != null)
		{
			CollectiblePermissionChanged(entityId, arg2: false);
		}
	}
	...
	private void Gathering([NotNull] GatheringData data)
	{
		...
		string text = data.CanGateringWithThisTool(currentTool);
		if (text == null)
		{
