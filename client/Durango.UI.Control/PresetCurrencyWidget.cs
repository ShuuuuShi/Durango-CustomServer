...
public class PresetCurrencyWidget : UIWidget
{
	...
	protected bool IsButtonActive => _chargeButton.activeSelf || _infoButton.activeSelf;
	...
	{
		EventType? eventType = ((_currencyType != Currency.Invalid) ? new EventType?(EventType.Currency) : ((!string.IsNullOrEmpty(_voucherId)) ? new EventType?(EventType.Voucher) : (_skillPoint ? new EventType?(EventType.SkillPoint) : ((_warpRushStoneType == ResourceType.Invalid) ? ((EventType?)null) : new EventType?(EventType.WarpRush)))));
	...
	private static bool IsShopEnabled()
	{
		...
		return purchasableList != null && GameSystem<MenuSystem>.Instance().IsEnabled(MenuType.Shop);
	}
	...
	public static void ChargeCurrency(Currency currency)
	{
		...
		GameSystem<ShopSystem>.Instance().GetPurchasableCommodities(delegate(List<Durango.Logic.Shop.Commodity> list)
		{
			...
			foreach (Durango.Logic.Shop.Commodity item in list)
			{
				...
				{
					flag = false;
					if ((item.SalesTag & Shared.Purchaser.Tags.Representative) != Shared.Purchaser.Tags.None)
					{
						commodity = item;
						break;
					}
	...
	public virtual void Init()
	{
		if (!_isInit)
		{
			...
			component.SetAnchor(componentInParent.gameObject, 0, 0, 0, 0);
