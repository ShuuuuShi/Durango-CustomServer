...
public abstract class KScrollViewBase : MonoBehaviour
{
	...
	private void OnDragFinished()
	{
		...
		else if (currentOffset > MaxOffset + (float)_scrollEndPadding - (float)_dragOnLastMargin && DragFinishedOnLast != null)
		{
			DragFinishedOnLast();
		}
		if (_alwaysIntOffset)
	...
	{
		MoveToOption value = new MoveToOption
		{
			Type = MoveToType.Offset,
			Offset = offset,
			RestrictWithinPanel = restrictWithinPanel,
			OnFinish = onFinish,
			Instant = instant
		};
	...
	public void MoveToVisibleArea(int index, bool instant, float beginPadding = 0f, float endPadding = 0f, bool restrictWithinPanel = true, Action onFinish = null)
	{
		MoveToOption value = new MoveToOption
		{
			Type = MoveToType.VisibleArea,
			Index = index,
			BeginPadding = beginPadding,
