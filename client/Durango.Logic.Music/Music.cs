...
public class Music
{
	...
	private static Music CreateFromMML(IEnumerable<string> tracks)
	{
		...
		foreach (string track in tracks)
		{
			...
			IEnumerable<KeyValuePair<double, MML.Note>> enumerable = MMLParser.ToNotes(track, out var tempo);
			if (music == null)
			{
				music = new Music();
				music.Division = 120;
			...
			foreach (KeyValuePair<double, MML.Note> item3 in enumerable)
			{
				if (!(item3.Value.Volume <= 0f))
				{
					...
					if (num >= 21 && num <= 108)
					{
						Note item = new Note
						{
							Tick = music.TimerToTick((float)item3.Key),
							Midi = num,
							On = true,
							Volume = Mathf.Clamp01(item3.Value.Volume)
						};
						Note item2 = new Note
						{
							Tick = item.Tick + music.TimerToTick((float)item3.Value.Length.TotalSeconds),
							Midi = num,
							On = false
						};
	...
	public Messages.Music ToMessage()
	{
		...
		return new Messages.Music
		{
			Name = Name,
			Duration = duration,
			Data = SnappyCodec.Compress(ToBytes())
		};
	}
