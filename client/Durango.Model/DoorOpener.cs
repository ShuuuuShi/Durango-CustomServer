...
public class DoorOpener : MonoBehaviour
{
	...
	{
		_fullyOpen = false;
		float initialYaw = _doorTarget.localRotation.eulerAngles.y % 360f;
		initialYaw = ((!(initialYaw > 180f)) ? initialYaw : (initialYaw - 360f));
		float maxYaw = initialYaw;
		float elapsed = 0f;
		while (true)
		{
			maxYaw = ((initialYaw != 0f) ? Mathf.MoveTowards(maxYaw, 0f, Time.deltaTime * 90f * (maxYaw / initialYaw)) : 0f);
