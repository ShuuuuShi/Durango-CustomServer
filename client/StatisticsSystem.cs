...
public class StatisticsSystem : GameSystem<StatisticsSystem>
{
	...
	public int Level => Statistics.HasValue ? Statistics.Value.Level : (-1);

	public int Exp => Statistics.HasValue ? Statistics.Value.Exp : (-1);
	...
	{
		return (!Statistics.HasValue) ? 1 : Statistics.Value.ResistanceLevels.Get(type, 1);
	}

	private int GetResistanceExp(Derived type)
	{
		return Statistics.HasValue ? Statistics.Value.ResistanceExps.Get(type, 0) : 0;
	}
	...
	{
		return (_resistanceExpCaps != null) ? _resistanceExpCaps.Get(type) : default(ResistanceExpCap);
	}
	...
	public int GetAverageResistanceLevel()
	{
		...
		return (KUtility.GetSize(Statistics.Value.ResistanceLevels) == 0) ? 1 : ((int)Math.Round(Statistics.Value.ResistanceLevels.Average((KeyValuePair<Derived, int> pair) => pair.Value)));
	}
	...
	private void Start()
	{
		...
		Connections.Frontend.On(delegate(ResistanceExpCaps msg, PacketHeader header)
		{
			...
			if (ResistanceExpCapUpdated != null)
			{
				ResistanceExpCapUpdated();
			}
		});
	...
	public Durango.Logic.LearningGuide.AdviceCategory GetAdviceCategory(int index)
	{
		...
		return (index >= AdviceCategories.Length) ? null : AdviceCategories[index];
	}
	...
	public void GetAdviceCategoryNotification(string categoryId, out bool on, out Durango.Logic.Notification.Type type)
	{
		...
		Durango.Logic.LearningGuide.Advice[] array = advices;
		foreach (Durango.Logic.LearningGuide.Advice advice in array)
		{
