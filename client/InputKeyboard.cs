...
public class InputKeyboard : InputDispatcher<InputKeyboard.Message>
{
	...
	public void Process()
	{
		...
		foreach (KeyValuePair<KeySet, InputCommand> item in _keyMap)
		{
			...
			if (key.Modifiers == pressedModifier && (currentLayer & key.Layers) > Layer.None)
			{
				...
				if ((key.Trigger & Trigger.Down) != Trigger.None && WinInput.GetKeyDown(key.Code))
				{
					trigger = Trigger.Down;
				}
				else if ((key.Trigger & Trigger.Up) != Trigger.None && WinInput.GetKeyUp(key.Code))
				{
					trigger = Trigger.Up;
				}
				else if ((key.Trigger & Trigger.Press) != Trigger.None && WinInput.GetKey(key.Code))
				{
	...
	public static string KeyToCaption(KeyCode keyCode)
	{
		...
		case KeyCode.Keypad9:
			return ((int)(keyCode - 256)/*cast due to constrained. prefix*/).ToString();
		default:
		{
			string text = NGUITools.KeyToCaption(keyCode);
			return (!string.IsNullOrEmpty(text)) ? text : keyCode.ToString();
		}
		}
	}
	...
	private void InitDefaultKey()
	{
		...
		_keyMap.SafeAdd(KeyCode.G, Layer.GamePlay, InputCommand.Mount);
		...
		_keyMap.SafeAdd(KeyCode.K, Layer.Menu, InputCommand.Skill);
		_keyMap.SafeAdd(KeyCode.M, Layer.Menu, InputCommand.WorldMap);
		_keyMap.SafeAdd(KeyCode.C, Layer.Menu, InputCommand.Recipe);
		_keyMap.SafeAdd(KeyCode.Z, Layer.Menu, InputCommand.ShowLastAddedItem);
		...
		_keyMap.SafeAdd(KeyCode.J, InputCommand.Quest);
	...
	{
		Type typeFromHandle = typeof(InputCommand);
	...
	public List<KeySet> GetKeySetList(InputCommand command, Trigger trigger)
	{
		...
		IEnumerable<KeySet> source = keySetList.Where((KeySet keyset) => (keyset.Trigger & trigger) != 0);
