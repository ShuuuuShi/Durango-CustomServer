	...
	public void Set()
	{
		...
		Clusters.RequestAccounts(GameManager.GatewayUrl, delegate(Account account)
		{
			...
			else
			{
				...
				int num = 0;
				for (int size = KUtility.GetSize(list); num < size; num++)
				{
					if (!(GameManager.PlayerId == list[num].PlayerEntityId))
					{
						CreatePlayerInfoButton(_scroll.Nodes, list[num].PlayerEntityId);
					}
				}
