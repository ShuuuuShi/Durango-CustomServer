...
public class PioneerPointPopup : TooltipBase
{
	...
	protected override void FillData()
	{
		...
		{
			_nodes.BeginLoad();
			for (int num = 0; num < KUtility.GetSize(pioneerCostExchangeRate.Rates); num++)
			{
				PioneerRate pioneerRate = pioneerCostExchangeRate.Rates[num];
				...
				float num2 = info.DailyExchangedPoints.Get(pioneerRate.Rate, 0f);
				UILabel uILabel2 = next.FindComponent<UILabel>("Points");
				uILabel2.text = string.Format("{0:0.#} / {1}", num2, (pioneerRate.Point < 0) ? "∞" : pioneerRate.Point.ToString());
				float fillAmount = ((pioneerRate.Point <= 0) ? 0f : (num2 / (float)pioneerRate.Point));
