...
public class SearchOption
{
	...
	public SearchProducts ToMessage()
	{
		return new SearchProducts
		{
			...
			NestedTags = GetNestedTag(),
			Category = ((MainCategory == null) ? null : MainCategory.Id),
			SubCategories = ((SubCategory == null) ? new string[0] : new string[1] { SubCategory.Id }),
			Price = ((!Price.Min.HasValue && !Price.Max.HasValue) ? ((PriceRangePredicate?)null) : new PriceRangePredicate?(Price)),
			Level = ((!Level.Min.HasValue && !Level.Max.HasValue) ? ((RangePredicate?)null) : new RangePredicate?(Level))
		};
	}
}
