...
public static class BotBridge
{
    ...
    // [แก้เอง 5 ก.ย. 2026] เปิดเป็น public เพราะ RuntimeInitializeOnLoadMethod อย่างเดียวไม่พอ —
    // BotBridge ไม่มีใครอ้างถึงเลย Unity จึง strip ทิ้งตอน build (ยืนยัน: เกมรันแล้วไม่มี log
    // "[BotBridge] listening" และไม่เปิดพอร์ต) ⇒ GameManager.OnAwake เรียกตรง ๆ อีกทาง
    ...
    /// <summary>
    /// [แก้เอง 5 ก.ย. 2026] โหมดออฟไลน์ถูกถอดออก — โปรเจกต์นี้มีเซิร์ฟจริงอยู่แล้ว
    /// ของเดิมเรียก Server.BeginServer/GetIslandPort ซึ่งเป็นของที่โปรเจกต์เก่า patch เพิ่มไว้
    /// ไม่มีในซอร์สต้นฉบับของ NEXON (BeginServer เป็น private) ⇒ ตัดทิ้งแทนที่จะไปแก้ต้นฉบับ
    /// </summary>
    private static void StartOffline(string key)
    {
        UnityEngine.Debug.LogWarning("[BotBridge] โหมดออฟไลน์ถูกถอดออก — ต่อเซิร์ฟจริงที่ 8190 แทน");
    }
    ...
    private static void AppendAnimals(StringBuilder sb)
    {
        ...
        bool first = true;
        {
            // [แก้เอง] ต้นฉบับเก็บ _animals เป็น private (AnimalManager.cs:13) — ของเดิมเข้าถึงตรง ๆ ได้
            // เพราะโปรเจกต์เก่าแก้ต้นฉบับไว้ ที่นี่ใช้ FindObjectsOfType แทนเพื่อไม่ต้องแตะไฟล์ของ NEXON
            AnimalBehavior[] animals = UnityEngine.Object.FindObjectsOfType<AnimalBehavior>();
            foreach (AnimalBehavior a in animals)
            {
                if (a == null) continue;
                if (!first) sb.Append(',');
                first = false;
