...
public static class Cheats
{
	public static Item? MakeItem(string prototypeId, int level)
	{
		...
		Item value = new Item
		{
			...
			Durability = new Gauge(1f, 0f, new GaugeNode[1]
			{
	...
	public static AppearArtifact? MakeAppearArtifact(string[] arguments, out AddOns? addons)
	{
		...
		Building.Blueprint blueprint = GameSystem<RecipeSystem>.Instance().RecipeContainer.GetBlueprint(int.Parse(s));
		if (blueprint == null)
		{
			return null;
		}
		...
		for (int i = ((!flag) ? 2 : 3); i < arguments.Length; i++)
		{
			...
			Building.BlueprintSlot blueprintSlot = blueprint.Slots.FirstOrDefault((Building.BlueprintSlot blueprintSlot2) => blueprintSlot2.Id == slotId);
