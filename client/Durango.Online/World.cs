...
public class World
{
	...
	private List<byte[]>[,] CreateByteMap(byte[] bytes, int stride)
	{
		...
		{
			int num2 = i * stride;
			int x = BitConverter.ToUInt16(bytes, num2);
	...
	{
		if (ArtifactDisappeared != null)
		{
			ArtifactDisappeared(aa);
		}
	}
	...
	public void DestroyNatural(Point2 tile)
	{
		if (RemoveNaturalFromGarden(tile))
		{
			...
			if (NaturalDestroyed != null)
			{
				NaturalDestroyed(tile);
			}
			Save();
		}
	}
	...
	private Chunk CreateChunk(int chunkX, int chunkY)
	{
		Chunk result = new Chunk
		{
			...
			Garden = _chunkData[chunkX, chunkY].Garden
		};
