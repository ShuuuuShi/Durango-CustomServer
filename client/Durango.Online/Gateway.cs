...
public class Gateway
{
	...
	public Gateway(GameServer gameServer, WorldContext worldCtx, PlayerContext playerCtx)
	{
		...
		_webServer.GetRoute["/knock"] = delegate(HttpListenerRequest request, Dictionary<string, string> postData)
		{
			...
			string text = request.QueryString.Get("platform");
			text = ((text != null && text.Equals("iPhonePlayer", StringComparison.OrdinalIgnoreCase)) ? "ios" : ((text == null || !text.Equals("Android", StringComparison.OrdinalIgnoreCase)) ? "windows" : "android"));
			jObject["assetbundle_index_url"] = $"http://assetbundles.k.nexon.com/{arg}/{text}/Info.5.2.1.json";
			jObject["assetbundle_url_root"] = $"http://durango-assetbundles.akamaized.net/{arg}/{text}/";
		...
		_webServer.PostRoute["/sessions"] = delegate(HttpListenerRequest request, Dictionary<string, string> postData)
		{
			...
			string text = postData.Get("player");
			if (!string.IsNullOrEmpty(text))
			{
				playerContext = Json.Read<PlayerContext>(text);
	...
	private WebServer.RouteFunction UnhandledUrl(string url)
	{
		...
		if (url.StartsWith("/terrains/1/ocean"))
		{
			return delegate
			{
				...
				{
					Content = _gameServer.World.GetChunkOcean(point2FromUrl)
				};
			};
		}
