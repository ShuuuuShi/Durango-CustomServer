...
public class CombatSystem : GameSystem<CombatSystem>
{
	...
	public bool CombatMode
	{
		...
		set
		{
			...
			if (flag)
			{
				...
				if (ChangedCombatMode != null)
				{
					ChangedCombatMode(_combatMode);
				}
			}
		}
	}
	...
	private void Awake()
	{
		...
		Connections.Frontend.On(delegate(BattleScenario msg, PacketHeader header)
		{
			...
			foreach (Damaged damaged in damages)
			{
				Connections.Frontend.Handle(12u, damaged, default(PacketHeader));
			}
		});
		...
		Observable<DamageableEntity> target = _target;
		target.Changed = (Action<DamageableEntity>)Delegate.Combine(target.Changed, (Action<DamageableEntity>)delegate(DamageableEntity damageableEntity)
		{
			SelectBattleTarget msg = default(SelectBattleTarget);
			if (damageableEntity != null)
			{
				msg.EntityId = damageableEntity.GetEntityId();
				msg.Tile = damageableEntity.GetTile();
			}
	...
	private void OnUseAction(BattleAction usedAction, DamageableEntity target)
	{
		...
		if (BattleActionUsed != null)
		{
			BattleActionUsed(usedAction);
		}
	}

	private void OnBattleActionCanceled(BattleAction action)
	{
		if (BattleActionCanceled != null)
		{
			BattleActionCanceled(action);
		}
	}
		...
		else
		{
			...
			foreach (BattleAction battleAction2 in actionSlots)
			{
				if (battleAction2 != null)
				{
					...
					if ((num2 ?? double.MaxValue) > (deactivateUntil ?? double.MaxValue))
					{
						num2 = deactivateUntil.GetValueOrDefault();
					}
					if ((num2 ?? double.MaxValue) > (battleAction2.ActivatedUntil ?? double.MaxValue))
					{
	...
	public void SelectTarget(string entityId)
	{
		...
		DamageableEntity target = DamageableEntities.Get(entityId, out var isAlly);
		if (isAlly)
		{
			ClearTarget();
		}
		...
		if (KUtility.GetSize(battleAction.Data.AttackInfo) > 0 && (target == null || !target.IsAlive))
		{
			...
			foreach (DamageableEntity enemy in DamageableEntities.GetEnemies())
			{
				if (!(enemy == null) && enemy.IsAlive)
				{
					...
					if (sqrMagnitude < (num ?? float.MaxValue))
					{
						num = sqrMagnitude;
						target = enemy;
					}
	...
	private void OnVehicleProjectileFired(VehicleProjectileFired msg, PacketHeader header)
	{
		...
		if (VehicleProjectileFired != null)
		{
			VehicleProjectileFired(msg);
		}
	}
