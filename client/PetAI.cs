...
public class PetAI : StateBasedAI<PetAI.State>
{
	...
	private IEnumerator ChaseDoing()
	{
		...
		while (true)
		{
			...
			float dt = Time.time - prevTime;
			prevTime = Time.time;
			float distance = Maths.Make2D(base.Master.transform.position - base.transform.position).magnitude;
			...
			Vector3 destPos = CalcChasePosition(base.Master);
			Vector3 dir = Maths.Make2D(destPos - base.transform.position).normalized;
			TargetAnimal.TurnToYaw(Maths.CalcYaw(dir), bSnap: false);
			Vector3 velocity = dir * _vehicle.MoveSpeed;
			Vector3 prevPos = TargetAnimal.CurrentPosition;
			Vector3 nextPos = ProcessCollisionWithSliding(TargetAnimal.CurrentPosition, velocity * dt);
			if ((nextPos - prevPos).magnitude / dt / _vehicle.MoveSpeed < 0.7f)
			{
				if (isMoving)
				{
			...
			else
			{
				...
				Locate(nextPos, randomYaw: false);
			}
			yield return null;
		}
		base.CurState = State.SpawnNearMaster;
	...
	private IEnumerator ReturnDoing()
	{
		...
		float item2 = runMotion.Item2;
		targetAnimal.CrossFade(item, fadeTime, loop: true, 0f, item2);
		Vector3 dir = Maths.Make2D(base.transform.position - base.Master.transform.position).normalized;
		if (dir == Vector3.zero)
		{
			dir = Vector3.right;
		}
		Vector3 returnPos = base.transform.position + dir * 3000f;
		float prevTime = Time.time;
		while (true)
		{
			float dt = Time.time - prevTime;
			...
			dir = Maths.Make2D(returnPos - base.transform.position).normalized;
			TargetAnimal.TurnToYaw(Maths.CalcYaw(dir), bSnap: false);
			Vector3 velocity = dir * _vehicle.MoveSpeed;
			TargetAnimal.CurrentPosition += velocity * dt;
			yield return null;
