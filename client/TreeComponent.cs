...
public class TreeComponent : NaturalComponent
{
	...
	private IEnumerator CoLoot()
	{
		...
		{
			float num;
			float elapsedTime = (num = Time.realtimeSinceStartup - startFellingTime);
			if (!(num < 4f))
			{
				break;
			}
			float alpha = 1f - elapsedTime / 6f;
			base.Sprite.SetAlpha(alpha);
			float rotFactor = elapsedTime / 4f;
			rotFactor = Mathf.Pow(rotFactor, 6f);
			if (rotFactor > 1f)
			{
				rotFactor = 1f;
			}
			tree.transform.localRotation = Quaternion.Euler(0f, 45f, -45.5f * rotFactor);
			yield return null;
		}
		...
		while (true)
		{
			...
			if (num < 4.2f)
			{
				float alpha2 = 1f - elapsedTime / 6f;
				base.Sprite.SetAlpha(alpha2);
				float rotFactor2 = (elapsedTime - 4f) / 0.19999981f;
				tree.transform.localRotation = Quaternion.Euler(0f, 45f, 0f - (45f + 0.5f * (1f - rotFactor2) + -0.2f * rotFactor2));
				yield return null;
				continue;
			}
			break;
		...
		while (true)
		{
			...
			float alpha3 = 1f - elapsedTime / 6f;
			base.Sprite.SetAlpha(alpha3);
			float rotFactor3 = (elapsedTime - 4.2f) / 0.10000038f;
			tree.transform.localRotation = Quaternion.Euler(0f, 45f, 0f - (45f + -0.2f * (1f - rotFactor3)));
			yield return null;
		}
		...
		while (true)
		{
			...
			float alpha4 = 1f - elapsedTime / 6f;
