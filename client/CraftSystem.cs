...
public class CraftSystem : GameSystem<CraftSystem>
{
	...
	public void TechSupport()
	{
		...
		CraftQueueItem craftQueueItem = new CraftQueueItem
		{
			Recipe = _slotContainer.Recipe,
			Materials = dictionary,
			Tool = ((!_slotContainer.Tool.ToolRequired) ? null : _slotContainer.Tool.GetSelectedItem()),
			Workbench = _slotContainer.Workbench
		};
		...
		{
			if (TechSupportSucceed != null)
			{
				TechSupportSucceed(target.Item);
			}
		});
	}
	...
	private void MakeCraftQueue()
	{
		...
		for (int i = 0; i < quantity; i++)
		{
			...
			CraftQueueItem item = new CraftQueueItem
			{
				Recipe = _slotContainer.Recipe,
				Tool = ((!_slotContainer.Tool.ToolRequired) ? null : _slotContainer.Tool.GetSelectedItem()),
				Workbench = _slotContainer.Workbench,
	...
	public void DoNextCraftQueue()
	{
		...
		{
			CraftQueueItem craftQueueItem = (LastCraftData = _craftQueue.Dequeue());
			Craft msg = new Craft
			{
				RecipeId = craftQueueItem.Recipe.Id,
				Materials = craftQueueItem.MakeMaterialIds(),
				ToolItemId = ((craftQueueItem.Tool != null) ? craftQueueItem.Tool.Id : string.Empty),
				Workbench = ((!(craftQueueItem.Workbench == null)) ? new PropKey?(craftQueueItem.Workbench.GetPropKey()) : ((PropKey?)null))
			};
			RegisterPostCraftEvents(Connections.Frontend.Send(msg), craftQueueItem.Recipe.Id);
		}
	}
	...
	private void RegisterPostCraftEvents(ReplyMessageHandlerRegistrar handler, string recipeId)
	{
		...
		{
			if (EntrustedCraftStarted != null)
			{
				EntrustedCraftStarted();
			}
			DoNextCraftQueue();
		}).On(delegate(Crafted crafted, PacketHeader header)
		{
			if (crafted.Result == Result.BigFailure)
			{
				if (CraftFailed != null)
				{
					CraftFailed(recipeId, crafted.ActionInfo);
				}
			}
			else if (CraftSucceed != null)
			{
				CraftSucceed(recipeId, crafted);
			}
			DoNextCraftQueue();
		})
	...
	public static void DyeingEstimate(Artifact workbench, [NotNull] ItemData item, [NotNull] ItemData dye, ColorChannel channel, [NotNull] Action<CraftEstimation?> onResult)
	{
		...
		if (recipe != null)
		{
			...
			EstimateCraft msg = new EstimateCraft
			{
				RecipeId = recipe.Id,
				Materials = dictionary,
