...
public class StatusEffects
{
	...
	{
		if (StatusEffectAdded != null)
		{
			for (int i = 0; i < _changedList.Count; i++)
			{
				if (_changedList[i].Value)
				{
					StatusEffectAdded(EntityId, _changedList[i].Key);
				}
			}
		}
		if (StatusEffectRemoved != null)
		{
			for (int j = 0; j < _changedList.Count; j++)
			{
				if (!_changedList[j].Value)
				{
					StatusEffectRemoved(EntityId, _changedList[j].Key);
				}
			}
		}
		_changedList.Clear();
		if (StatusEffectsUpdated != null)
		{
			StatusEffectsUpdated(this);
		}
	}
