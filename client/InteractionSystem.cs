...
public class InteractionSystem : GameSystem<InteractionSystem>
{
	...
	private static readonly HashSet<int> NearHashSet = new HashSet<int>();
	...
	public InteractionObject Target
	{
		...
		private set
		{
			...
			if (InteractionTargetSelected != null)
			{
				InteractionTargetSelected(Target);
			}
		}
	}
	...
	private void Start()
	{
		...
		Durango.Utils.Singleton<GameManager>.Instance().MainSceneLoaded += delegate
		{
			...
			_drawTimer.Started += delegate(PredictTimer timer)
			{
				...
				iconProgressGauge.AddIcon(CurrentMenu.Icon);
			};
			...
			_washBodyTimer.Started += delegate(PredictTimer timer)
			{
				...
				IconProgressGauge iconProgressGauge = Durango.Logic.Timer.Timer.Play<IconProgressGauge>(timer.Timer);
				iconProgressGauge.AddIcon(IconMap.Get(Interaction.WashBody));
			};
			...
			_drinkWaterTimer.Started += delegate(PredictTimer timer)
			{
				...
				IconProgressGauge iconProgressGauge = Durango.Logic.Timer.Timer.Play<IconProgressGauge>(timer.Timer);
				iconProgressGauge.AddIcon(IconMap.Get(Interaction.DrinkWater));
			};
	...
	{
		Collider[] array = Collisions.OverlapSphere(pos, checkDistance, mask, out var count);
		NearHashSet.Clear();
	...
	public void SetInteractionTarget(InteractionObject target)
	{
		...
		bool result = false;
		if (PreTouchTarget != null)
		{
			PreTouchTarget(target, ref result);
			if (result)
			{
				return;
			}
	...
	public void RefreshInteractionMenu()
	{
		...
		if (PostTouched != null)
		{
			PostTouched(_menuList, Target);
		}
		_menuList.Apply();
		if (OnTouchItemSucceed != null)
		{
	...
	private static void DefaultContextActionFinder(List<InteractionMenuData> result)
	{
		...
		if (role != Role.Tutorial && role != Role.Personal && GameManager.ClusterMode == Mode.Online)
		{
	...
	private static void BiomeContextAction(List<InteractionMenuData> result)
	{
		...
		foreach (KeyValuePair<string, PutInContainerInfo> item2 in putInContainerInfos)
		{
			if (item2.Value.Biomes != null && Array.IndexOf(item2.Value.Biomes, biome) != -1)
			{
