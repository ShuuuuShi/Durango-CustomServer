...
public class BuildSystem : GameSystem<BuildSystem>
{
	...
	private void OnArtifactBuiltMsg(ArtifactBuilt msg, PacketHeader header)
	{
		...
		if (!(artifact == null) && PlayerBehavior.LocalPlayer.EntityId == msg.BuilderId && BuildFinished != null)
		{
			BuildFinished(artifact);
		}
	}
	...
	private void OnArtifactCompletedMsg(ArtifactCompleted msg, PacketHeader header)
	{
		...
		if (!(artifact == null))
		{
			...
			if (artifact.FounderId == PlayerBehavior.LocalPlayer.EntityId && BuildCompleted != null)
			{
				BuildCompleted(artifact);
			}
		}
	}
	...
	public void PutMaterials(Action onSccuess)
	{
		...
		PutMaterialsIntoArtifact msg = new PutMaterialsIntoArtifact
		{
			EntityId = _slotContainer.Artifact.EntityId,
			Tile = _slotContainer.Artifact.WorldTile,
			Materials = _slotContainer.CreateFirstMaterialsDictionary(canFinished: false)
		};
	...
	public void Build()
	{
		...
		BuildArtifact msg = new BuildArtifact
		{
			EntityId = _slotContainer.Artifact.EntityId,
			Tile = _slotContainer.Artifact.WorldTile,
	...
	public void Remodeling()
	{
		...
		SoundManager.PlayEvent("ui_button_build");
		string toolItemId = _slotContainer.GetToolItemId();
		RemodelArtifact msg = new RemodelArtifact
		{
			EntityId = _slotContainer.Artifact.EntityId,
			Tile = _slotContainer.Artifact.WorldTile,
			SlotId = _slotContainer.Blueprint.Id,
			Materials = _slotContainer.CreateFirstMaterialsDictionary(canFinished: false),
			ToolItemId = toolItemId
		};
		BuildTimer.Play(Connections.Frontend.Ping + 10f);
	...
	{
		GetArtifact msg = new GetArtifact
		{
