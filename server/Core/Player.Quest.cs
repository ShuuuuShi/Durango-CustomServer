...
public partial class Player
{
    ...
    /// <summary>**ค่าของเรา** — เพดาน id ต่อคำขอเดียว (เกมจริงถามครั้งละ 1)</summary>
    private const int MaxQuestIdsPerRequest = 256;
    ...
    private void HandleGetQuestStateMsg(GetQuestState msg, uint seq)
    {
        ...
        {
            // ⚠️ [6 ก.ย. 2026] เพดานจำนวน — client ที่ถูกแก้ส่ง array ยาวเท่าไรก็ได้
            // แล้วเซิร์ฟจะสร้าง Dictionary ตามนั้นและ pack กลับทั้งก้อน
            // ของจริงเกมถามครั้งละ 1 id (client/Durango.Logic/QuestSystem.cs:207-209)
            // 256 เผื่อไว้กว้างมากแล้วสำหรับทุกกรณีที่เกมทำจริง
            int taken = 0;
            foreach (string questId in msg.QuestIds)
            {
                if (string.IsNullOrEmpty(questId)) continue;
                if (++taken > MaxQuestIdsPerRequest)
                {
                    Console.WriteLine($"[เควส] {Short(EntityId)} ถามสถานะเควสเกิน {MaxQuestIdsPerRequest} ตัว — ตัดที่เพดาน");
                    break;
                }
